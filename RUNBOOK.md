...
`--skip-gate` merges without re-running discrimination and logs a warning (`apps/cli/src/run.ts:617-620`) — a deliberate override, never a default. The gate always runs in FAKE mode (`apps/cli/src/run.ts:641`), precisely so a nightly merge is not conditional on a token being valid at 3am.
