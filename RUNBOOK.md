...
| `MEMHTML_LLM_BASE_URL`                 | —                     | An OpenAI- and Anthropic-compatible LLM proxy's origin, e.g. `http://127.0.0.1:4000` for an agentgateway listener. Set, every model call — the sleep models and the entity extractor on `/v1/messages` and `/v1/chat/completions`, embeddings on `/v1/embeddings`, the consolidator agent on `/v1/messages` — leaves through it instead of going to Bedrock directly, and no AWS credential is consulted. A set-but-malformed value fails at startup naming the variable.                                                                                                                                                                                                                               |
...
18 tools and 3 resources over this same repo (`apps/mcp/src/tools.ts`). Sleep is deliberately absent from the tool surface: it is a cron and operator action producing a reviewable branch, not something an agent triggers mid-conversation.
...
The phase refuses to move a file that a phase whose write carries a DECISION already touched on this branch, and it reads that set from the per-commit diffs of the run's own commits whose `Memhtml-Phase` trailer is not in `SWEEP_PHASES` (`packages/sleep/src/touched.ts`). A uniform sweep is exempt: `confidence-decay` restamps every eligible file in the corpus, so counting it would pin essentially every candidate. When reading it fails the set widens to the whole range rather than emptying, and a set neither read can establish leaves the phase reporting a reason before its first model call. Each refusal is its own count beside the `refused` total, so `refusedTouched` against `refusedNewDirCap` against `keptInbox` says which constraint actually bound the night — read those before concluding the model declined the work.
...
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `no consolidator bound`                             | `MEMHTML_LLM=off`, or neither Bedrock credentials nor an LLM proxy in the environment. The phase was never able to run. | Nothing, if that was intended. Otherwise export `AWS_BEARER_TOKEN_BEDROCK` (or the SigV4 pair), or point `MEMHTML_LLM_BASE_URL` at a proxy.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
