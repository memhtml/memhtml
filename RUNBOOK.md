...
| `MEMHTML_CONSOLIDATOR_COMMAND_TIMEOUT_MS` | `60000`               | The wall-clock limit, in milliseconds, on each shell command the consolidator agent runs in its sandbox. A command that reaches it is killed on its own worker thread; the agent is told to narrow the search and the turn continues. A set-but-unparseable value fails startup rather than falling back.                                                                                                                                                                 |
