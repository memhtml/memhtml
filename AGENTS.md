...
| `memhtml apply` | — | `--file` `--continue-on-error` `--detect-conflicts` `--session-id` `--prompt-id` `--turn-uuid` | `batch.applied` |
| `memhtml read` | <path> | `--session-id` | `memory.detail` |
| `memhtml search` | <query> | `--type` `--workspace` `--tag` `--entity` `--include-archived` `--limit` | `memory.hits` |
| `memhtml recall` | <query> | `--type` `--workspace` `--tag` `--entity` `--include-archived` `--budget` | `recall.pack` |
