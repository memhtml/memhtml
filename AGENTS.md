...
- `--timeout-ms` (int) — Wall-clock bound on the script. Exceeding it is `exitCode` 124 with `timedOut: true`, not an error envelope. Values above 600000 are REFUSED, not clamped. _(default `30000`)_
- `--sha` (string) — The commit to mount, 4-40 hex characters, materialized as a detached worktree. Defaults to HEAD. Never the live working tree, whose gitignored .memhtml/index.db a worktree omits.
