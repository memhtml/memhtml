 ...
 * dependency as a shape — `packages/sleep/src/env.ts:18-23` records that a runner which built its
 ...
 * the seeding path broke and {@link manifestFor} records the mechanism for. It has a consequence worth
 ...
 *
 * **No longer the only thing keeping the agent off the network, and still not optional.**
 ...
 * module mints (`run-auth.ts`). So the two controls answer different questions — loopback bounds who
 ...
 * sandbox handle, and the only channels a client has to it are a model turn — which is what the
 * superseded seeding path used, and what made the transcripts model-mediated — or a build-time
 * `agent/sandbox/workspace/**` bake, which cannot carry per-run values
...
/**
 * How often the readiness poll asks. 100ms, against a 1.79s measured start: ~18 probes, each one a
 ...
 * three times running is not a race any more — it is a box with something claiming ports faster than
 * this can use them, which a fourth attempt would not fix.
 */
...
export interface ConsolidatorOptions {
   ...
   *
   * **This is how transcripts reach the agent**, and it is REQUIRED — the one non-optional option —
   * because a client without it has no mechanism for its own job. It is `MEMHTML_TRACE_ROOT`, resolved in
   ...
   * values, and this object is built once per client. That was the blocker recorded here before —
   * `baseSha` is on `PhaseEnv` (`packages/sleep/src/env.ts:80`) while the repository path is
   * `RootsShape.memhtmlRoot` in the CLI's composition root — and it is resolved by widening the CALL's
   ...
   * pin a worktree for the process's life and release it never.
   *
   * The manifest's `linkedMemories` field covers the load-bearing half of the same question without a
   * mount, by naming the memories the corpus already links to each session — which is the specific
...
export const guestPathFor = (input: {
   ...
   * measured above. An ABSOLUTE result means the two paths share no root at all — `relative` returns
   * the target verbatim across Windows drives — which would append an absolute path after the mount
   * prefix.
   */
 ...
 * filesystem, and that is the point of building a `MountableFs` here rather than calling `stat`.
...
> =>
  Effect.gen(function* () {
     ...
     * unreachable for that reason — the honest answer, since a mount that cannot be composed here
     * cannot be composed in the server either.
     */
 ...
 *
 * That is not incidental. `.memhtml` holds no session content and neither does a model context message
 * this client composes; a manifest that quoted a first prompt to be "helpful" would put session text
 * back into the same place it was just removed from. Session ids, paths, spans, counts, and the
 * corpus paths already linked to a session — nothing that was written INSIDE a transcript.
 *
 ...
 * node_modules/eve/dist/src/internal/nitro/host/start-production-server.js) — so passing an explicit
 * port is not working around the CLI, it is doing the same step one process earlier, where the answer
 ...
 *
 * A REAL check, and specifically not a sleep: the health route is a framework route eve registers on
 ...
 *
 * Readiness is a poll of that constructed origin rather than a stdout watch, and the distinction
 * matters beyond taste: the listening line is printed by the CLI wrapper AFTER its own health wait
 * succeeds, so a stdout watch would be waiting on eve's wait. Polling directly is the same signal one
 * layer down. It is also not a sleep — see {@link healthy}.
 *
 ...
 *
 * Requires `eve build` to have run — `.output/` is what `eve start` serves. That is `build:agent`,
 * deliberately outside the turbo graph (§6), so this reports a typed
  ...
  Effect.callback<ServerHandle, StartAttemptFailure>((resume) => {
    ...
    const child = spawn(
      ...
      ["exec", "eve", "start", "--host", LOOPBACK_HOST, "--port", String(port)],
      {
         ...
         * files eve loads INSIDE the spawned process — `agent/sandbox/sandbox.ts` for the mounts,
         ...
         *
         * The secret is the one thing in this environment that is a credential, and it is why the
         * remaining exposure is a reader of THIS CHILD's environment: `/proc/<pid>/environ` for the
 ...
 * to ports, not that the agent build is broken.
 *
 ...
 *
 * It names the manifest and the count and it does NOT list the session ids. That is a change from the
 * seeding-era message and it is deliberate: the ids are in the manifest, on disk, where the model
 * reads them from the same file it reads the paths from — and a context that also carried them as
 ...
 * transcript, then the analysis turn. Both the extra turn and its cost are gone — the transcripts are
 * already on a read-only mount before the server is spawned, so the first model call this run makes is
...
const runTurn = (
  ...
  Effect.gen(function* () {
     ...
     * follow a cross-origin redirect" (types.d.ts:65-70). Nothing should redirect a loopback POST, which
     * is the point — if something does, the token stops here rather than travelling.
     */
    ...
    // Decoded with `onExcessProperty: "error"`, and that is the load-bearing option for the
 ...
 * process is spawned or any file read. The Bedrock provider is lazy — it constructs happily with
 * no credentials and fails only at the first request — so without this check a credential-free
  ...
  return {
    consolidate: ({ transcripts }) =>
      Effect.gen(function* () {
         ...
         * read would cost a model call for a result already known — and `analyzedSessionIds` is `[]`
         ...
         * enforces it (`agent/channels/eve.ts`) — and a leaked temp directory is a manifest of a past
 ...
 * per call rather than a fixed path under `tmpdir()`, because two sleep runs — or a run and a
 * hand-driven probe — sharing one path would have each overwrite the other's manifest while both
 * mounts stayed live.
 *
 ...
 * scanned — which would mount a tree whose paths no `traces` row names.
 */
