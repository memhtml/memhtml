 ...
 * path — `.erpaval` lesson `performance/synchronous-detector-on-untrusted-write-path.md` and §8 of
 ...
 * by eve into the SERVER process — a different build than this package's `tsc -b`, reaching `src/` by
 * relative path exactly as `agent/sandbox/sandbox.ts` reaches `mount.ts`. Nothing outside this app
 * consumes it: the client mints and signs, the channel verifies, and there is no third caller.
 */
