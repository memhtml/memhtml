 ...
 *   `MountableFs.mount` rejects `.`/`..` segments itself, but it silently normalizes everything else
 *   — a relative path, a doubled separator, a trailing slash — so the declared path and the effective
 *   mount would differ and a typo would mount somewhere other than where it reads. Note `/mnt/memhtml/`
 ...
 *
 * ## `mountPoint: "/"` on the nested overlay is load-bearing, and a file count cannot check it
 *
 ...
 *
 * `readOnly: true` is enforced, not advisory: a write through the composed filesystem throws
 * `EROFS: read-only file system`, and through `Bash` the command throws the same. `..` traversal out
 * of a mount and an absolute `/etc/hostname` both fail — the overlay resolves a guest path against
 * its own root and returns nothing outside it. And `allowSymlinks` defaults to FALSE, so a symlink
 * under a mounted root is not followed: any real path traversing one is rejected. That is the safe
 * direction and it is worth knowing it costs reachability — `~/.claude/skills/*` holds symlinks to
 * directories outside the trace root, and those read as absent inside the sandbox.
 *
 ...
 * spawned it — two processes, so the roots have to cross a process boundary, and the spawn
 ...
 *
 * A MALFORMED variable throws, and that split is the point: absent is the normal case for a sandbox
 * with nothing to mount, while a variable that is present and unparseable means the spawner meant to
 * mount something and this process would silently run without it. A sandbox that quietly lost its
 * corpus answers questions about an empty corpus, which reads as a finding about the data.
 *
