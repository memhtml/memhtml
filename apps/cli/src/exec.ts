 ...
 * census in 598ms, and an edge walk resolving 410/410 edges into 201 chains — longest 8 hops — in one
 * execution at 430ms.
 *
 ...
 *
 * 30s, which is `maxJsTimeoutMs`'s own default in just-bash 3.2.0 (`dist/limits.d.ts:83`) — named here
 * rather than inherited so the value appears in `memhtml manifest` and in `AGENTS.md`, where an agent
 * budgeting a call can read it. The measured work is far below it: the whole 305-file corpus parses in
 * 640ms, so 30s is roughly 45x the cost of a full-corpus pass.
 */
export const DEFAULT_TIMEOUT_MS = 30_000

/**
 * The bound is capped, and the cap is the point.
 *
 * `maxJsTimeoutMs` is the ONLY thing standing between a runaway guest loop and a `memhtml exec` that never
 * returns: the guest is a QuickJS worker with no host-side reaper of its own, and an unbounded script
 * would hold the CLI process open indefinitely. Probed 2026-08-09 with `maxJsTimeoutMs: 700` against
 * `for(;;){n++}`: exit 124 at 724ms, "js-exec: Execution timeout: exceeded 700ms limit".
 */
 ...
 *
 * Not extra budget for the script — `maxJsTimeoutMs` still cuts it off at the requested value. This is
 * the margin that decides WHICH of the two bounds reports, and therefore whether `stderr` carries the
 * message naming the limit. See {@link runExec} for the measured table.
 */
 ...
 * precisely so `tsc` does not see it: it is guest source, its imports resolve against guest paths
 ...
 *
 * Resolved from `import.meta.url` and not from `process.cwd()`, so `memhtml exec` works from any directory
 * — `dist/exec.js` sits one level under the package root, which is where `guest/` is.
 */
 ...
 * package's `sideEffects` field. So the file `pnpm` installs is already loadable in the guest verbatim,
 ...
 * whenever `source` carries a `toString` — which is exactly what cheerio does when it attaches its
 ...
 * ONE of the two wordings ever reaches a live report — which makes the other branch unfalsifiable
 ...
 * - `maxExecutionTimeMs` fires: `bash: js-exec exceeded its execution deadline` — no "timeout" in it
 *
 * So a pattern matching only `/timeout/` reports `timedOut: false` on a script that WAS cut off, which
 * is what the first version of this did. `aborted` covers `bash: execution aborted`, which is what an
 * `AbortSignal` produces (probed) — not a path this module takes today, and cheap to classify correctly
 * if it ever is.
 *
...
export interface ExecReport {
  ...
  /** The script's exit code. Non-zero is reported, not raised — see {@link runExec}. */
 ...
 *
 * ## Non-zero exit is a REPORT, not a failure
 *
 * A script that throws, or that exits 1 deliberately, comes back as a successful envelope carrying
 * `exitCode` and `stderr`. The alternative — mapping a guest exit onto the CLI's own exit 1 — would
 ...
 * network commands at all — `Bash.d.ts:80`: "Network commands (curl, wget) are registered when either
 * `fetch` or `network` is provided." Probed 2026-08-09 (`scripts/probe-sandbox-egress.mjs`): `curl` is
 * exit 127 "command not found", and the guest's `fetch` — which IS a function, so a `typeof` check on
 * the global proves nothing — refuses on CALL with "Network access not configured." The consolidator's
 * sandbox does reach the network, because eve passes `dangerouslyAllowFullInternetAccess`; egress is
 * decided by whoever calls `new Bash()`, so it is decided here, for this runtime, by omission.
 *
 * ## Two opt-ins, one taken
 *
 * `javascript` is on because `js-exec` IS the feature. `python` is NOT, and neither is `network`. Both
 * are off by default in just-bash — a deliberate default worth preserving as an explicit decision
 * rather than inheriting silently, because a future edit adding `python: true` for one recipe would
...
export const runExec = (
  ...
  Effect.gen(function* () {
     ...
     * `OverlayFs` — the gotcha a file count cannot catch, because all three spellings expose the same
     * number of files at three different prefixes — and it validates roots eagerly, so a bad
     ...
     * the JS worker. But which one fires first changes the DIAGNOSTIC, probed 2026-08-09 on a
     ...
     * the limit — is what reaches `stderr`. The margin is a grace period, not extra budget: the script
     * is already cut off at `timeoutMs`, and the shell's bound exists only to catch the case where
     * `js-exec` itself fails to stop.
     */
    const bash = new Bash({
      fs: filesystem,
    ...
    const result = yield* Effect.tryPromise({
      ...
      // A THROWN failure from `bash.exec` is the runtime's, not the script's: just-bash reports a
 ...
 *
 * A `Failure` return and not a raised error, for the reason `applyText` has one: an unreadable input
 * path is a USAGE error the caller fixes by changing the call, so it must reach exit 2, and only
 ...
 * /mnt/memhtml/.memhtml/index.db 'select count(*) …'` returned the row, exit 0. So read-only is no barrier to a
 * READER, and mounting the live root would hand every script the ranked planes this command is scoped
 * to exclude — through a door no `memhtml exec` flag opens.
 *
 ...
 * commit. Verified by construction rather than argued — the worktree probe in
 * `apps/cli/tests/exec.test.ts` asserts `.memhtml` is not present and that the guest's own `sqlite3` finds
 * nothing to open. That makes containment a property of WHAT IS MOUNTED, with the read-only flag as a
 * second layer, rather than resting on a flag that a reader can read straight through.
 *
 * The pin is also what makes an answer reproducible: `sha` rides back in the report, so the same
 * traversal over the same tree is one `--sha` away, and an uncommitted edit is invisible — which is
 * the honest behaviour for a command whose whole output is a claim about a corpus state.
 *
...
export const execCommand = (input: {
  ...
  Effect.gen(function* () {
     ...
     * in the operator's repository — not a temp directory the OS reclaims. The release runs on the
     * script's failure, on a timeout, and on an interrupt, which a `finally` around the happy path
     * would not cover.
     */
