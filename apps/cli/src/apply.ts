 ...
 * because AC-6-4's contract is that the whole file is judged before any op executes — which makes
 ...
 * malformed line is a usage error — the caller wrote a bad file, the corpus is fine.
 */
 ...
 * `archive` without a format break, and an unknown value is refused WITH this list rather than
 * ignored: a file of `{"op":"wrote",…}` lines that applied nothing and exited 0 is the silent
 ...
 * snake_case names (`apps/mcp/src/tools.ts`), which is the point: an agent that learned the field
 * names from one door can write a JSONL file for the other without translating.
 */
const SCALAR_FIELDS = {
  title: "title",
  type: "memoryType",
...
/** `op` is the discriminator, not a `WriteParams` field, so it is legal but never mapped. */
 ...
 * markup valid) — checked per op, reported per op, and NOT duplicated here, because a second copy of
 * the type vocabulary at this door is a second thing to update when the vocabulary moves.
 */
const opAt = (record: Record<string, unknown>, line: number): WriteParams | Failure => {
   ...
   * bad op reports that op and not the whole file. What this branch owns is the CLAIM: the JSONL wire
   ...
   *
   * Skipping this no longer lands a bad file — `@memhtml/html` constraint 1 refuses an empty `<mark>`, so
   * the render gate would REFUSE the op instead of committing a file with an empty `files.gist`. The
   * derivation is what makes a prose line valid in the first place, not the thing standing between a
   * missing claim and a silent write.
   */
 ...
 * which prefix landed. Refusing first costs nothing — no service has been touched at this point.
 *
 * Blank lines are skipped rather than refused, because a file written by `printf '%s\n'` or a heredoc
 * ends in one, and the LINE NUMBERS still count them: an error naming line 7 has to mean the seventh
 ...
 * {@link readStdin}, so `bin.ts` needs no edit and a test supplies the text directly. The
 * alternative — reading stdin in `bin.ts` — would make the entry point parse argv to discover whether
 ...
 *
 * snake_case here and camelCase in `memory.written` is a real inconsistency, and it is the SPEC's
 * (D6 names `commit_sha`). Taken deliberately, because it makes this payload byte-comparable with
 * `memory_write_batch`'s over MCP — an agent that has parsed one has parsed the other, and a batch
 * result is the payload most likely to be handled by shared code across the two doors.
 *
 * Absent fields are `null`, not missing. `deduped` and `skipped` are always booleans. An agent
...
const opPayload = (report: BatchOpReport) => ({
   ...
   * one shape. Null when the flag was off, when nothing matched, or when the claim has no frame shape
   * — and an op carrying a conflict was still WRITTEN, which is the whole contract: the field is a
