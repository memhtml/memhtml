...
The server publishes eighteen tools and three resources over this same repo (`apps/mcp/src/tools.ts`). Sleep stays off that tool surface deliberately: a curation run is a cron and operator action that produces a reviewable branch, and an agent should not start one mid-conversation.
