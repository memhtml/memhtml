...
| `MEMHTML_OPENAI_PROMPT_CACHE`          | `off`                 | How the OpenAI sleep model's chat-completions requests ask Bedrock to treat prompt caching: `off` sends the field that turns it off, `implicit` sends nothing. Read on the direct path and the proxy path alike. See [turn off the OpenAI lane's cache writes](#turn-off-the-openai-lanes-cache-writes).                                                                           |
...
## Turn off the OpenAI lane's cache writes

The OpenAI sleep model (`gpt-5.6-sol`) is called on the chat-completions body, and on Bedrock that body is subject to [prompt caching](https://docs.aws.amazon.com/bedrock/latest/userguide/prompt-caching.html) in its default _implicit_ mode: Bedrock places an automatic cache breakpoint on the latest message, and every request whose prompt clears the 1,024-token minimum writes that prompt to the cache at 1.25x the uncached input rate. A write only pays for itself when a later request reads the same prefix back. The eight sleep prompts are 189 to 496 tokens each, so the only prefix that repeats never reaches the minimum, and the only prefix that does reach it, the system prompt plus one unique batch, is never seen twice. Measured on a gateway access log on 2026-09-11, every OpenAI-lane call reported `cache_write_tokens` about equal to its input and `cached_tokens: 0`: a premium with no read to pay it back.

So by default memhtml sends `prompt_cache_options: {mode: "explicit"}` with no breakpoints, which Bedrock documents as "the request does not use prompt caching or incur cache-write charges". `MEMHTML_OPENAI_PROMPT_CACHE` is the switch:

| Value      | On the wire                                | When                                                                                                                                                                                                                      |
| ---------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `off`      | `prompt_cache_options: {mode: "explicit"}` | The default. Bedrock, direct or behind a proxy, for prompts whose prefix will not repeat at 1,024 tokens or more.                                                                                                         |
| `implicit` | no caching field                           | A prefix that will repeat and clear the minimum, or an OpenAI-compatible endpoint that rejects the Bedrock-only field (plain OpenAI accepts `prompt_cache_key` and `prompt_cache_retention`, not `prompt_cache_options`). |

The field was probed live on 2026-09-11 against `bedrock-runtime.us-east-1.amazonaws.com/openai/v1/chat/completions` with `global.openai.gpt-5.6-sol`: accepted on Chat Completions and honored, `cache_write_tokens: 0` on a 3,424-token prompt that wrote 2,641 tokens without it. The Anthropic lane is untouched. It names its cache prefix with `cache_control` on the system block and has no automatic breakpoint to turn off.

One caveat for the proxy path: the field only works if the proxy forwards it. An LLM-aware gateway that parses the chat-completions body into its own model and rebuilds it upstream drops the keys it does not know, and one that adds its own cache breakpoints per route writes regardless of what the client sent. Read the gateway's usage fields after the change: `cache_write_tokens` (or the gateway's `cache_creation` counter) at zero on a prompt over 1,024 tokens is the confirmation, and anything else is the proxy's configuration, not this variable.
