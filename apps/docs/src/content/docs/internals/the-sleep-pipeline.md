...
**Figure 2: four of the ordering edges, drawn over the six phases that carry them.** Eleven of the seventeen phases appear nowhere in this graph, so the order among them is arbitrary and could change without consequence. The three hexagons mark the phases here that call a model, and each one's edge is about that call: `compress` archives a member only when the model names it as absorbed, and never a dated `episodic` record (one carrying a `day` facet), which it summarizes, links from the canonical, and stamps `part_of` the canonical instead (retention triage and dedup-merge are not covered by that exemption), `dedup-merge` gates both of its dependents on a fold the model proposed and the veto allowed, and `entity-resolution` runs before `person-links` so a person's aliases have already collapsed before a file is minted for them.
...
**`preflight` gates the whole run: every one of the sixteen phases after it.** A failed preflight therefore commits nothing. It establishes the four preconditions the rest of the night reads, and each failure makes every later commit wrong rather than merely unhelpful: a dirty tree means a later phase commits the operator's own uncommitted bytes under sleep's trailers, an `EmbedModelMismatch` is a half-migrated vector space that degrades every cosine while each individual vector stays well-formed, an `IndexStale` is an index a rebuild emptied and did not finish repopulating, so every later phase's counts describe a corpus fragment, and a `VectorCoverageLow` is a vector plane covering under half the chunks, so every cosine pass compares a sample of the corpus against itself and calls the rest unique. All four end in the one outcome per-phase isolation is no defense against, a corrupt night with a green report. Between the hard floor of `0.5` and `MEMHTML_VECTOR_COVERAGE_FLOOR` (default `0.95`) preflight warns, records `vectorCoverage` in its counts, and continues; the check applies only when the vector plane is in use (some vector exists, or an embedder is bound), so a lexical-only store under `MEMHTML_EMBED=off` is not refused. A store that embeds nothing under the default `MEMHTML_EMBED=on` (no credential) is refused, and the failure names `MEMHTML_EMBED=off` as the remedy.
...
The three predicates do not read the same text (`MergeText` in `packages/domain/src/merge.ts`). The numeric predicate reads the one-line claim (the `gist`) alone, because a body is where a memory cites its evidence and two honest statements of one fact almost never cite the same date, ticket id, or exit code: over the whole article it vetoed 349 of 349 candidates across two consecutive runs. Negation and variant qualifiers read the claim and the article joined, because their markers are rare, load-bearing words a body does not carry incidentally, and a polarity flip stated only in the body is a contradiction the veto has to see.

The in-batch role guard (`mergeOutcomes`, `packages/domain/src/merge.ts`) fixes each committed pair's drop as dropped and its keeper as kept for the batch. A dropped file cannot be dropped again or later be a keeper, and a keeper cannot later be dropped: given the pairs `(gf, a)` and then `(b, gf)`, both would commit, `gf` absorbs `a` and is then archived into `b`, so `a`'s content is superseded into a file the same batch destroyed. A keeper may keep again, so a model group of N members folds all N-1 of its duplicates into the oldest in one run. The guard used to claim both roles of a committed pair, which folded one member per group per run and reported the rest as vetoes; the phase now counts guard skips under `roleGuarded`, apart from `vetoed`.

The veto, the self-merge check, the role guard, and the per-run cap are a post-filter over **every** proposal, including a model's (`packages/domain/src/merge.ts:14-18`). With a model bound `dedup-merge` mines a recall-oriented candidate set at 0.86, unions it with the frame-key exact matches, builds connected components over that union, and asks the model to partition each component into merge groups. Every pair a group implies is then routed through `mergeOutcomes`, so the set of pairs that _can_ be committed does not widen when a model is bound: it is the same predicate over a different candidate set. A model that groups a claim with its own negation is refused by the predicate that refuses a blind cosine.
...
Batching makes the role guard carry more rather than less. One answer names several groups, and two groups overlapping on one path is exactly the `(gf, a)` then `(b, gf)` chain arriving from one call instead of from two runs.
...
`arc-synthesis` splits triage from execution, because one call asked to both choose and write produces content for arcs it should have skipped, and the writing is the expensive half (`packages/sleep/src/phases/arc-synthesis.ts:25-28`). `compress` archives a member only when the model names it as absorbed, and never a dated `episodic` record (one carrying a `day` facet), which it summarizes, links from the canonical, and stamps `part_of` the canonical instead (retention triage and dedup-merge are not covered by that exemption), so an omitted member stays active, which is the safe outcome (`packages/sleep/src/phases/compress.ts:24-27`).
...
- **Placement triage.** A deep-only phase proposes an existing `areas/<topic>` or `resources/<topic>` directory — or at most five new ones per run — for each inbox memory even deep grouping could not attach, and `git mv`s the confident placements, rewriting inbound hrefs in the same commit. `keep-inbox` is the model's refusal and the ordinary answer. It never moves a file that a phase whose write carries a DECISION already touched on this branch, because that would fold the other phase's edit into a rename a reviewer reads elsewhere — but a UNIFORM SWEEP is exempt, and the distinction is what makes the phase able to act at all. `confidence-decay` restamps `memhtml-confidence` on every eligible file in the corpus, so a guard that counted it would pin essentially every candidate and refuse essentially every move. The set is therefore the union of the per-commit diffs of the run's own commits whose `Memhtml-Phase` trailer is not in `SWEEP_PHASES` (`packages/sleep/src/touched.ts`, `packages/sleep/src/contract.ts`). Membership names what a phase decides rather than how wide its diff is: `person-links` splices one `<link>` and leaves the article's bytes identical, and it still pins, because the edge it authored is not in the index this phase reads to repair inbound hrefs. The trailer is the source rather than any in-memory record, so a `sleep resume` — where the earlier attempt's phases exist only as commits — reads the same set. A read that fails widens to the whole range instead of emptying, and a set neither read can establish leaves the phase reporting a reason before its first model call. Every refusal class is its own count beside the `refused` total, so a night that moved little says which constraint bound it.
...
The plan's verdict has three values, and the third is the honest one. `would-change` means a counted signal is non-zero. `no-signal` means every counted signal is zero, every phase whose candidates cannot be counted has an empty input, AND the counts describe the tree the caller has checked out, so a run would reach nothing — the one state in which a caller may skip. Everything else is `unknown`: `dedup-merge` and `relationship-mining` select candidate PAIRS from an n-by-n scan, so counting their candidates IS the scan, and they report their input cardinality with an explicit unknown rather than a zero. A predicate that said "no effect" because it did not look would be worse than no predicate.

That last clause is the third way to not look, and the cheapest. Every count is `index.db`'s, and `index.db` is a disposable projection a caller is invited to delete — so a clone, a `git pull`, or an `rm` answers every aggregate with zero for a corpus in any state at all. The plan therefore takes the tree's own `HEAD` and publishes `indexFresh` (the predicate `memhtml status` reports) beside `indexedCommit` (the commit the numbers are about), and a `false` there makes `no-signal` unreachable. The same guard covers a failed read: an unreadable database fails the watermark too, so the zero an unread aggregate falls back to cannot become permission to skip.
