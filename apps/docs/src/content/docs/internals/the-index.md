...
`embedMissing` (`packages/index/src/indexer.ts`) requests vectors only for chunks that have none or that carry a vector from another model. On the update path the pending scan is scoped to the batch's own chunk ids, so `update --embed` embeds only its own batch. A store-wide gap, left by `--no-embed` runs or by failed embed calls, is closed by `memhtml index embed`, the bare `embedMissing()` whose scan stays unscoped and which writes nothing but vectors; `rebuild --embed` runs the same unscoped scan after reprojecting. A rebuild keeps the vectors it already has: `truncateForRebuild` stashes the `embeddings` rows in the configured space and restores every one whose content-addressed chunk id the reprojection produced again, so only a model change empties the vector plane, and `rebuild --no-embed` over a store that carries vectors is refused with `ERR_REBUILD_NO_EMBED_REFUSED` unless `--force` is given.
