...
It then fails with a typed `WriteConflict` carrying the path and both shas (`packages/contracts/src/errors.ts:19`). That reaches an agent as `ERR_WRITE_CONFLICT` plus the suggestion to re-read and reapply (`apps/cli/src/errors.ts:130-133`).
...
An agent write is one commit against one file and can be reconciled after the fact. A curation run rewrites metadata across the whole corpus, so starting it against uncommitted work would mix a human's in-progress edit into a machine's sixteen commits, with no way to tell them apart in the diff.
