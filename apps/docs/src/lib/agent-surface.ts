 ...
 * produces `//runtime/index.md` — a protocol-relative URL naming a *host* called `runtime` — which is
 * a bug shipped in the wild, and it is silent: the href parses, resolves to nothing, and looks right
 * in source.
 */
...
export const referencePrompt = (page: PageReference): string =>
  [
    "Read this page of the memhtml documentation, then answer questions about it.",
 ...
 * two of these relations are what several vendors ship, and the third is ours.
 */
export const discoveryLinks = (
  ...
  },
  {
    ...
    // in HTTP `Link:` headers — a channel this host gives no control over.
    rel: "index",
    type: "text/markdown",
  ...
  {
    // OURS. `llms-full-txt` has no registration and no other adopter; it is named here so that a
    // later reader does not mistake it for a standard they failed to find.
    rel: "llms-full-txt",
    type: "text/markdown",
