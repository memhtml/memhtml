 ...
 *
 * The defect this closes, measured on this site: 450 links across the `.md` twins pointed at
 * `/internals/…` rather than `/memhtml/internals/…`, and every one was a 404 for the agent following
 * it. The HTML was correct throughout, which is precisely why it went unnoticed — the two surfaces are
 * built from different things. `starlight-base-path` rewrites the rendered tree, while
 * `starlight-md-txt` builds each raw route from the page's Markdown source, so a link is correct on
 * whichever surface its producer touched and wrong on the other.
 *
 * Rewriting the store's entry bodies does not reach it: the raw route re-reads a file-backed page from
 * disk, so only the loader-injected pages would change. The emitted output is the one place both kinds
 * of page have converged, which is why this runs on `astro:build:done` rather than earlier.
 *
 ...
 * no-op, so this composes with an incremental build instead of compounding.
 */
