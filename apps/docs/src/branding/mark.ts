 ...
 * what the mark is. `tests/branding.test.ts` decodes the committed artifacts independently and
 * compares pixels, not bytes — a zlib that packs the same image differently is not a drift.
 *
 * Everything under `branding/` is imported with its real `.ts` extension, where the rest of this
 * package writes `.js`. `write-icons.ts` is run by node directly, which resolves a specifier
 * literally and has no `.js`-to-`.ts` fallback; one spelling that both node and Vite resolve is
 * cheaper than two spellings for one module. `allowImportingTsExtensions` is on in Astro's own base
 * tsconfig, so this is also what typechecks.
 */

/** The unit count of the mark's coordinate system. Sixteen, so a 16px raster is 1 unit per pixel. */
export const GRID = 16

/** The three palette values the mark uses, each carrying its measured ratio against the ground. */
export const PALETTE = {
  /** Warm paper: the ground a printed specification is read on. */
  paper: "#FCFBF7",
  /** 16.83:1 on paper. */
  ink: "#1A1A18",
  /** 7.09:1 on paper — never #EE0000, which is 4.38:1. */
  normative: "#B00018"
} as const
 ...
 *
 * The red band is authored full-bleed (`x: 0`, reaching `y = GRID`) on purpose: an inset rule would
 * leave a one-pixel paper margin at 16px that reads as a rendering fault rather than as a margin.
 */
export const MARK: ReadonlyArray<MarkRect> = [
  { x: 0, y: 0, w: GRID, h: GRID, fill: PALETTE.paper, role: "ground" },
  { x: 3, y: 3, w: 10, h: 2, fill: PALETTE.ink, role: "document name" },
  { x: 3, y: 7, w: 6, h: 2, fill: PALETTE.ink, role: "status line" },
  { x: 0, y: 13, w: GRID, h: 3, fill: PALETTE.normative, role: "masthead rule" }
]

/** The accessible name every rendering of the mark carries. */
export const MARK_LABEL = "memhtml"
 ...
 *
 * Edges are placed with `Math.round` rather than floor/ceil so a size that is not a multiple of
 * `GRID` — 180, the Apple touch size — distributes its remainder instead of accumulating it on one
 * side. Every rectangle stays at least one pixel tall at every size this ships.
 */
 ...
 *
 * The site optimises no images and Astro runs `passthroughImageService()` because sharp is out of
 * reach of a resolve from this app's root, so there is no rasteriser to borrow. The mark is four
 * axis-aligned rectangles, which is well inside what `node:zlib` plus a CRC table can encode: one
 * IHDR, one IDAT of unfiltered RGBA8 scanlines, one IEND.
 */
 ...
 * these sizes and stated because it is the trap in this format. PNG payloads inside ICO have been
 * read by every shipping browser since IE11; the alternative, an uncompressed DIB with a separate
 * AND mask, buys nothing here.
 */
...
/** The Apple touch icon's side length. iOS scales anything else and softens the rules doing it. */
export const APPLE_TOUCH_SIZE = 180

/**
 * The PNG icon's side length, and the masthead logo on every social card.
 *
 * It ships at exactly the size it is drawn at because the card renderer resamples with a linear
 * filter: scaling the 180px icon down to 48 would soften every rule the geometry went to the trouble
