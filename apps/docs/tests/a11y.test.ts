...
/**
 * The accessibility gate: axe-core over four representative pages, plus four probes for things axe
 * does not do or does not do reliably.
 *
 * Four pages rather than 93, and that bound is the reason this gate will still be here in a year — a
 * check whose cost grows with the corpus gets deleted the first time it makes someone wait.
 * `src/gates.ts` says why these four, and `tests/built-site.test.ts` proves the sample is not
 ...
 * reasons: four browser page loads instead of twenty, and the spacing probe has to restyle the page,
 ...
 * Label in Name is a Level A criterion and this site fails it, so the rule is switched on explicitly
 * rather than left to a default that would have hidden the finding.
 *
 * `scrollable-region-focusable` is switched OFF, and not because the defect it reports is acceptable.
 * Measured 2026-08-12 against `dist/`: five axe runs over one unchanged page, identical layout each
 * time (`pre.scrollWidth - pre.clientWidth` held at 286px through load, network idle, fonts ready and
 * a further 1.5s), and the rule reported a violation on one run of the five. A blocking gate cannot
 * hold a check that flips at 20%. The criterion is covered instead by `scrollableRegions` below,
 * which reads the same geometry directly and is deterministic, and which reports under the same rule
 * id so the baseline entry still governs it.
 */
const AXE_OPTIONS = {
  runOnly: {
    type: "tag" as const,
 ...
 * to look at and non-conforming to use. axe ships no rule for it — the criterion needs a focused
 * element and a hit test, which only a browser can supply.
 */
 ...
 * `role="img"` — so an inline `<svg>` with no role at all is invisible to it, and inline SVG with no
 * role is how Starlight draws every icon and how astro-d2 would draw a figure.
 *
 * A real DOM is required because the answer usually lives on an ancestor. Three ways to be decided,
 * each measured against this site: `aria-hidden` on the element or above it (Starlight wraps the
 * heading-anchor icon in `<span aria-hidden="true">`); an `aria-label`/`aria-labelledby` ancestor,
 * which supplies the accessible name so the SVG's own content is never consulted (the scroll-to-top
 * button); or `role="img"` with a name of its own that is not a placeholder.
 */
...
/**
 * SC 1.4.12 Text Spacing, which axe does not test at all — it cannot, because the criterion is about
 * what happens after the reader overrides the spacing, and axe reads the page as authored. The
 * overrides are the ones the criterion names verbatim.
 *
 ...
 *
 * Two further narrowings, each measured against this site rather than guessed. The scope is the BLOCK
 * containers the criterion is written about — a `<span>` inside a code line clipping while its
 * scrollable parent still holds the text is not a loss of content. And a visually-hidden element is
 * skipped: Starlight labels every heading anchor with a 1x1 `.sr-only` span whose whole purpose is to
 * overflow a one-pixel box, and reporting it would have made this probe cry wolf on all 93 pages.
 */
...
beforeAll(async () => {
  ...
  try {
    ...
    throw new Error(
      "could not launch Chromium. The browser is acquired by `mise run browsers`, which " +
        "`mise run install` already runs — run it if this is a fresh clone.",
      { cause }
    )
  }
  ...
  for (const path of AUDITED_PAGES) {
    ...
    expect(response?.status(), `${path} must be a page of the built site`).toBe(200)
    /*
     ...
     * elements the reader gets — measured: the inline-SVG probe found the button on some runs and
     * not others until this line existed.
     */
...
describe("the baseline is a ratchet", () => {
  ...
  it("audits the four declared pages and no others", () => {
