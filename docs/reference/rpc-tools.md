...
This repository ships an MCP server, `memhtml-mcp`, over stdio. It publishes fourteen tools and two resource templates, and it is the surface a coding agent calls to operate a memhtml root. The repository stores no memory of its own: the server acts on whatever root `$MEMHTML_ROOT` points the process at, and the same binary serves many roots.

The server is one Effect layer: `McpServer.toolkit(MemhtmlToolkit)` merged with the two resources, over the CLI's own app layer, on the stdio transport at protocol revision `v2025_06_18` (`apps/mcp/src/server.ts:39-54`). Sharing the CLI's layer is deliberate, so an agent's `memory_write` and an operator's `memhtml search` resolve to one database, one git root, and one vector space (`apps/mcp/src/server.ts:12-19`). Logs are pinned to stderr because stdout carries the NDJSON-RPC frames (`apps/mcp/src/server.ts:20-22`, `apps/mcp/src/bin.ts:7-13`).
...
Every tool declares `failure: ToolFailure`, a tagged error carrying a stable `ERR_*` code, a composed message, and suggestions phrased as tool calls the agent can make (`apps/mcp/src/failure.ts:32-39`). The declaration is what puts a failure on the branch that passes prose through. A tool that omitted it would have every failure, typed domain error included, rewritten to a generic internal-error sentence before the caller saw it (`apps/mcp/src/tools.ts:33-41`, `apps/mcp/src/failure.ts:8-16`).
...
**Output:** a `text/plain` body: an H1 of the title, the gist, then the article body text, joined by newlines. The markup is not returned; head metadata is `memory_read`'s job (`apps/mcp/src/resources.ts:28-32`). A missing path fails the read rather than resolving to an empty resource. This read bumps salience, through the same `readMemory` the `memory_read` tool calls, because naming one path is a chosen open (`apps/mcp/src/resources.ts:36-40`).
...
**Output:** a `text/html` body read from the root's tree at `<memhtmlRoot>/<SLEEP_REPORTS_DIR>/<date>.html`. The resource reads the tree rather than the database because the committed report is the durable artifact of a run and the `sleep_runs` row is reporting convenience (`apps/mcp/src/resources.ts:60-66`).
...
Soft-evicts a memory by `git mv` into `archive/<YYYY>/` with the archive stamps, so nothing is deleted and `git log --follow` reads straight through.
...
Supersedes a memory: writes the corrected version and archives the target in one commit, linked in both directions, never editing in place.

**Input:** `target_path`, `title`, and `reason` required; exactly one of `body` or `article_html`; `session_id` optional. The exclusive-or between `body` and `article_html` is enforced at the handler by `authored`, not by the schema, because a schema union would publish two near-identical full parameter shapes and name neither branch's problem on a decode failure (`apps/mcp/src/handlers.ts:107-136`).
...
Asserts an edge between two memories, written into the source file's head so it survives an index rebuild, and idempotent on the pair.
...
Pages through the corpus by facet, with a keyset cursor on the path so a page stays correct while a sleep cycle archives files.
...
Returns the memory graph around one path, to at most two hops, in both directions, including sleep-mined edges.
...
Reads one memory in full: head metadata, authored links, and the complete article body, and it is the only path to a `<details>` body that recall never quotes.

**Input:** `path` required, `session_id` optional. An explicit open counts as salience, which is why this tool declares `DatabaseService`: `readMemory` reaches the state plane through `bumpAccess`, so the dependency set makes the salience rule visible (`apps/mcp/src/tools.ts:412-417`). A search or recall hit does not move the plane.
...
Returns a context pack under a character budget: full bodies for what fits, one index line each for what does not, with arcs folded under their own envelope so a synthesis cannot crowd out the evidence behind it.
...
**Output:** `sections` with three arrays, plus `spent_chars`, `truncated`, and `degraded`. `arcs` and `memories` carry full bodies. `lateral` is the union of both folds' index lines, which is what did not fit the budget rather than a third retrieval arm; dropping it would make a truncated pack indistinguishable from a small corpus (`apps/mcp/src/handlers.ts:566-572`).
...
Records that a memory helped or misled, gated by a per-path cooldown so a replayed query cannot inflate a memory's ranking.
...
Runs ranked search over the corpus, fusing lexical, vector, recency, and salience arms with RRF and then diversifying.

**Input:** `query` required; the rest optional. `limit` defaults to 10 (`packages/index/src/retrieval.ts:29`). `memory_types` draws from the nine writable types. `entity` takes one reference in `type:name` form, the same spelling `memory_list` accepts and the same spelling a hit's `entities` publishes, so an agent chains by copying a value rather than reconstructing one (`apps/mcp/src/tools.ts:447-451`). `as_of` takes an ISO instant and returns what was believed valid at that moment, over the window `coalesce(valid_from, event_at, created_at) <= as_of < valid_until` (`apps/mcp/src/tools.ts:452-458`).

**Output:** `hits` plus four result-level fields. Each hit carries `snippet`, the best-matching chunk's text for this query or the file's opening chunk on the degraded path, truncated with a trailing ellipsis when cut. `superseded_by` is present and nullable so a client can tell "not superseded" from "this build does not report supersession" (`apps/mcp/src/tools.ts:485-491`). `degraded` is true when the vector arm did not fire. `arms` names the arms that ran. `scope_empty` is true when an `entity` scope narrowed the query and nothing survived, so an empty scoped result is attributable to the scope; the tool never widens a scope it could not satisfy (`apps/mcp/src/tools.ts:499-505`). Returning a path changes nothing on the access plane, because a hit is the ranker's guess rather than a chosen open.
...
Reports corpus health in one call: HEAD, dirty state, counts by type, edge totals, whether the index describes the current commit, and when sleep last ran.
...
**Output:** eight fields. `index_fresh` is true when the index's watermark is the current HEAD, which a row count cannot answer. `last_sleep` is null when no run is recorded, otherwise `run_id`, `status`, and `started_at` (`apps/mcp/src/handlers.ts:755-762`). This is the read the failure suggestions point an agent at when a write fails on a dirty tree, a git error, or a storage error (`apps/mcp/src/failure.ts:111-117`).
...
Writes one memory to the corpus, returning the existing path with `deduped: true` when an active memory already holds this exact content, in which case no file and no commit are created.

**Input:** `Schema.Struct(writeFields())`, thirteen fields shared with each `memory_write_batch` op (`apps/mcp/src/tools.ts:226-247`). Required: `title` and `memory_type`, the latter one of the nine writable types, since `arc` is system-written by the sleep cycle (`apps/mcp/src/tools.ts:44-45`, `packages/contracts/src/types.ts:34-40`). Optional: `body`, `article_html`, `path`, `workspace`, `tags`, `entities`, `importance`, `confidence`, `session_id`, `prompt_id`, `turn_uuid`. Exactly one of `body` or `article_html` must be supplied, enforced at the handler rather than in the schema, and a blank string counts as absent on both sides (`apps/mcp/src/handlers.ts:107-136`). On the prose path the first sentence becomes the `<mark>` claim and each blank-line paragraph becomes one `<p>`; on the markup path the caller owns the format, including the single `<mark>` rule, the closed element vocabulary, and the consequence that the first `<time datetime>` becomes the memory's event time that the recency arm ranks by (`apps/mcp/src/tools.ts:133-144`).
...
Writes many memories in one commit: every op is validated first, every surviving file is staged, and the batch commits and reindexes exactly once.

**Input:** `ops`, an array of `BatchOp`, which is `Schema.Struct(writeFields())` and therefore carries the same thirteen fields as `memory_write` including the same one-of-`body`-or-`article_html` rule (`apps/mcp/src/tools.ts:277`). Sharing the field record is what keeps the two published schemas from disagreeing when a field is added (`apps/mcp/src/tools.ts:213-225`). `continue_on_error` switches from the atomic default to best-effort. `detect_conflicts` adds a per-op `conflict` field and changes nothing about what is written. `consolidate` accepts the single literal `"last-wins"`, a one-value `Literals` rather than a boolean so the vocabulary can widen without a shipped `true` changing meaning (`apps/mcp/src/tools.ts:377-382`). `session_id`, `prompt_id`, and `turn_uuid` are batch-level provenance that an op's own value overrides.
...
**Input:** `session_id` and `path`, both optional, but at least one required. Both absent is refused rather than answered with every link ever recorded.
...
Finds past Claude Code sessions by what was asked in them, over a read-only index that stores pointers and capped heads rather than session content.
