...
Both metas serialize after `memhtml-needs-revision` and before the repeatable keys
...
All three are **report-only**: `--fix` repairs dangling hrefs and orphan `state.access` rows and nothing
...
The first two do not affect `healthy` (`:488`): they are facts about the work, not defects in the
