...
Each flow starts at an external event. The repository has no HTTP server and no queue consumer: every process here is either a CLI invocation of the `memhtml` binary or a tool call arriving on the MCP server's stdio transport (`apps/cli/src/bin.ts:5`, `apps/mcp/src/bin.ts:15`).

The three flows are the write verb, the read verb, and the second door onto the same store. Participant labels follow `architecture/module-map.md`: `cli`, `store`, `index`, `mcp`.

## Flow 1: Write one memory

An agent runs `memhtml write --title … --claim … --type semantic`. The process renders the file, refuses it if the format check finds a violation, asks the dedupe question before touching disk, commits, then brings the index up to the new commit.
...
7. The store renders the article, runs `checkMemory` and refuses on any violation, hashes the bytes, returns the existing path when the hash already exists, then writes the file, stages it with `git add`, and makes one commit `packages/store/src/store.ts:534`.
...
An agent runs `memhtml search "<prose>"`. The query is embedded when a credential is available, four reciprocal-rank-fusion arms are folded in one SQL statement, the fused pool is hydrated, MMR reorders it for diversity, and one more statement fetches the winning snippet per path.
...
2. `searchMemories` delegates to the retrieval service and records no access bump, so a ranked hit never feeds back into salience `apps/cli/src/operations.ts:927`.
...
An MCP client runs `memhtml serve mcp`, which spawns the `memhtml-mcp` server as a child process holding the client's own descriptors. A `tools/call` then reaches the same operation the matching CLI command calls.

1. `run` handles `serve mcp` before building the app layer, so the supervisor never opens the database the child exists to serve `apps/cli/src/run.ts:869`.
...
8. The handler renames the result fields back to snake_case for the wire, and any typed failure passes through `toToolFailure` so the stable code and its suggestions survive the one prose error channel MCP offers `apps/mcp/src/handlers.ts:537`.
