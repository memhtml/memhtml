...
`apps/cli` builds the `memhtml` binary declared at `apps/cli/package.json:8`, and it is also the composition root the MCP server imports rather than duplicating (`apps/cli/src/index.ts:2`). Every command an agent calls writes exactly one JSON envelope to stdout, whose `apiVersion`, `type`, and `code` fields are append-only discriminators an agent branches on (`apps/cli/src/envelope.ts:2`, `apps/cli/src/envelope.ts:12`, `apps/cli/src/envelope.ts:67`). Exit code 0 is success, 2 is a usage error the agent fixes by changing the call, and 1 is a runtime failure it fixes by changing the root or the environment (`apps/cli/src/envelope.ts:88`). The command table in `commands.ts` is the single machine-readable contract: `memhtml manifest` serves it, and `memhtml agents-doc` regenerates `AGENTS.md` from it (`apps/cli/src/commands.ts:111`, `AGENTS.md:1`).
...
`@memhtml/sleep` runs the nightly curation cycle over the root as fifteen git commits, each phase its own commit on a `sleep/<date>` branch (`packages/sleep/src/index.ts:2`). The fifteen phase names, their execution order, and their prerequisite graph live in one contract module so a phase name is not restated in a commit trailer, a `sleep_phases` row, and a `--phases` flag (`packages/sleep/src/contract.ts:2`, `packages/sleep/src/contract.ts:17`). Each commit carries a `Memhtml-Run` / `Memhtml-Phase` / `Memhtml-Counts` trailer block written in exactly one place, and `memhtml sleep resume` reads those trailers out of the branch's git log to decide what a run already did, so no journal table is load-bearing (`packages/sleep/src/commit.ts:10`). The package constructs none of its own services: every dependency arrives as a caller-supplied shape, which is what lets the whole test tier point a run at a temp-dir git repo (`packages/sleep/src/env.ts:10`).
...
`@memhtml/store` is the git-backed file store and the only writer to the root: every operation that changes the corpus is one commit, and every git failure is a typed value (`packages/store/src/index.ts:2`). The tree is the system of record and the index is derived from it, so an operation that changed a file without committing would leave the index describing a state git does not have (`packages/store/src/index.ts:2`). Git's `-z` plumbing formats are parsed by pure total functions in `plumbing.ts`, pinned against captured bytes so a truncated stream from a killed subprocess is a tested case rather than a crash (`packages/store/src/plumbing.ts:2`). `layout.ts` holds the scaffold `memhtml init` writes, including the PARA directories and the `merge=ours` driver registration, and it never creates the root implicitly so a typo in `MEMHTML_ROOT` is an error rather than a second empty root (`packages/store/src/layout.ts:41`, `packages/store/src/layout.ts:76`, `packages/store/src/layout.ts:15`).
...
`@memhtml/html` is the memory file format and its only implementation: parse, serialize, hash, and the surgical head editors (`packages/html/src/index.ts:2`). The format is semantic HTML5 over a closed vocabulary, and `vocabulary.ts` states that vocabulary as data, so the policy is a set of element and metadata names rather than a sanitizer with allow and deny logic (`packages/html/src/vocabulary.ts:2`, `packages/html/src/vocabulary.ts:150`, `packages/html/src/vocabulary.ts:252`). `constraints.ts` is the format check the store runs before it writes anything, so a refused write leaves the tree byte-identical (`packages/html/src/constraints.ts:351`, `AGENTS.md:62`). The `sha256` content hash over the normalized text of `<article>` is the dedup key and is invariant under head edits, which is what lets a metadata change avoid looking like new content (`packages/html/src/hash.ts:8`, `packages/html/src/hash.ts:19`).
...
`apps/mcp` builds the `memhtml-mcp` stdio server declared at `apps/mcp/package.json:8`, exposing fourteen tools and two resources over the same root the CLI reads (`apps/mcp/src/index.ts:2`). It imports `layerApp` from `apps/cli` rather than composing its own service graph, so both agent surfaces resolve to exactly one database file, one git root, and one vector space (`apps/mcp/src/server.ts:1`, `apps/cli/src/index.ts:2`). Sleep is deliberately absent from the tool surface: a run rewrites confidence across the corpus and creates a branch a human is expected to read, so `memhtml sleep run` is its only entry point and no tool fires it (`apps/mcp/src/index.ts:2`). `failure.ts` declares one wire error class whose message survives the MCP framework's rewrite of generic tool failures, so the prose an agent reads is the prose the tool wrote (`apps/mcp/src/failure.ts:5`).
...
`@memhtml/contracts` is the root of the dependency graph: it declares no `@memhtml/*` dependency of its own and twelve other workspace packages depend on it (`packages/contracts/package.json:6`). It owns the closed vocabularies every other module restates in SQL rather than in a second TypeScript copy: ten memory types, four PARA buckets, four non-mixing edge classes, and the memory and task relation sets (`packages/contracts/src/types.ts:18`, `packages/contracts/src/types.ts:63`, `packages/contracts/src/edges.ts:9`, `packages/contracts/src/edges.ts:19`). `paths.ts` is the path algebra, and every path it produces is the root-relative git-tree form with no leading slash (`packages/contracts/src/paths.ts:5`). A path is the memory's identity with no uuid anywhere in the system, so the slug rules in `slug.ts` carry the full burden of being stable, readable, and filesystem-safe (`packages/contracts/src/slug.ts:2`).
