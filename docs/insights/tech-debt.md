...
Fourth, pattern-level smells the reviewer chose to flag. These are duplicated agent-facing prose across the two doors, one oversized function, a phase whose destructive decision is never asserted, comment evidence citing paths a clone cannot read, and one stale ledger entry.
...
| 12 | An `article_html` op reports no conflicts at all, because the claim lives inside the caller's markup and the ops layer never parses it. Fixing it means moving the assist behind the store's own render. | wrong abstraction | L | `docs/backlog.md:131-135` |
...
| 15 | `pinCorpusSnapshot` is implemented and tested but wired to nothing. A snapshot is a git worktree that needs release, and the object that would hold it is built once per client, so the lifetimes do not match. | dead code adjacent | M | `apps/consolidator/src/mount.ts:258`, `apps/consolidator/src/client.ts:250-261` |
...
| 22 | `.claude/` is untracked and unignored, and it holds a 1.1 GB agent worktree, inside a project whose discipline is a clean git tree. | dead code adjacent | S | `.gitignore:12-19` |
...
| 24 | Any new MCP tool must declare `failure: ToolFailure` or its errors reach the agent as "internal server error". Only convention and wire tests enforce this, and no type signature does. | deprecated pattern | M | `docs/backlog.md:152-154` |
...
The same practice appears in the EARS requirements carrying `status: draft` in `spec/memhtml.symspec.json`. Eleven of them do, and each has a `verificationNote` naming its unfired condition. Verbatim examples:
