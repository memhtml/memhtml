...
- **`MEMHTML_LLM=off` reports its effect on a credential-free sleep run:** every model-calling phase reports `no model bound` and stays `ok` rather than red, and `entity-resolution` still runs its deterministic passes. `apps/cli/src/config.ts:55-60`, `packages/sleep/src/phases/conflict-detection.ts:56-59`.
