...
- **`MEMHTML_LLM=off` reports its effect on a credential-free sleep run:** the four phases with nothing to do without a model report `no model bound` and stay `ok` rather than red, while `dedup-merge` and `entity-resolution` fall back to their deterministic passes and still commit. `apps/cli/src/config.ts:55-60`, `packages/sleep/src/phases/edge-typing.ts:215-218`.
