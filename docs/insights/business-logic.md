...
- **`MEMHTML_LLM=off` reports its effect on a credential-free sleep run:** the four LLM phases report `no model bound` and stay `ok` rather than red. `apps/cli/src/config.ts:55-60`, `packages/sleep/src/phases/conflict-detection.ts:56-59`.
