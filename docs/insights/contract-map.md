...
Describes the source at 0.15.1 (main eb9d17a, 2026-09-22). Citations are `path:line` into that tree.
...
This repo's shared types are of two kinds: `effect` `Schema` declarations with a derived type (`export type X = typeof X.Type`), and plain TypeScript interfaces or type aliases. Every Shape block below quotes the declaration verbatim, whichever kind it is.
...
The sections are grouped by kind: the four vocabularies of `@memhtml/contracts` first, then the service and document shapes, then the agent-facing surfaces and the model boundary. Each of the five sections that states a dependent-file count measured it the same way, as every `.ts` file outside `tests/`, `test/`, `*.test.ts`, and `tests-integration/` whose text contains the module specifier (for the four vocabularies) or the bare identifier including its declaring file (for `DatabaseShape`), re-measured at eb9d17a with the same results: 40, 16, 19, 13 plus two migrations, and 18.
...
- `packages/store/src/store.ts:5-11` imports `DirtyTree`, `InvalidMemory`, `PathNotFound`, `StorageFailure`, `WriteConflict` and unions them into `StoreError` (`packages/store/src/store.ts:217-224`).
- `packages/index/src/database.ts:6` imports `StorageFailure` as the sole error type of every method on `DatabaseShape` (`packages/index/src/database.ts:61-93`).
- `packages/index/src/git-port.ts:1` imports `StorageFailure` as the port's one failure type (`packages/index/src/git-port.ts:50-75`).
...
A second typed error vocabulary sits outside both switches entirely. `apps/consolidator/src/contract.ts:806,814,829,851` declares four more `Schema.TaggedError` classes, `ConsolidatorCredentialsMissing`, `ConsolidatorUnavailable`, `ConsolidatorRunFailed` (whose `phase` is `invocation` or `turn`), and `ConsolidatorContractViolation`, each carrying a `reason`, and unions them as `ConsolidatorError` at `:859-863`. None of the four has an arm in `codeFor` (`apps/cli/src/errors.ts:41-79`) or in `mcpSuggestionsFor` (`apps/mcp/src/failure.ts:80-131`), so they are the seventh through tenth tags declared outside `packages/contracts/src/errors.ts`. Today that is harmless, because the union's only consumer is the sleep phase, which runs `consolidate` under `Effect.result` and turns a failure into a report value, `detail: "consolidator unavailable: <_tag>"`, at `packages/sleep/src/phases/trace-consolidation.ts:1018-1027`, so the tag never reaches `failureFor`. A CLI command or MCP tool that called `ConsolidatorShape` directly would report `ERR_UNKNOWN` with an empty suggestion array; add the four arms to `codeFor`, `messageFor`, `SUGGESTIONS`, and `mcpSuggestionsFor` before opening that door.

## The edge vocabulary and its derived class

This module declares four edge classes that do not mix, four rel vocabularies, and one total function from a rel to its class. It is imported by 16 non-test files across 4 packages and 2 apps, and the SQL restates the same vocabularies as CHECK constraints.

**Producer:** `packages/contracts/src/edges.ts:9-149`

**Consumer(s):**

- `packages/store/src/store.ts:4` imports `EdgeRel` and `relClassFor`; the store calls `relClassFor` at `packages/store/src/store.ts:1270` to decide which endpoint rule to enforce.
- `packages/index/src/project.ts:1` imports `relClassFor` and writes its result into the `edge_class` column at `packages/index/src/project.ts:366-368`.
...
- The indexer assumes `relClassFor` is total, so it writes the result straight into a NOT NULL column with no fallback (`packages/index/src/project.ts:366-368`). Totality comes from `relClassFor`'s final `return "provenance"` rather than from a check.
...
This module provides placement, archival, and normalization as pure total functions over repo-root-relative paths. It has 19 non-test importers across 4 packages and 2 apps, and it is the only module that decides where a memory lands in the root.

**Producer:** `packages/contracts/src/paths.ts:10-234`
...
- `packages/index/src/project.ts:371-375` re-implements the archive-path inverse as a local `originOf` instead of importing `originalPathFor`, so two copies of the same regex exist. Both match `^archive\/\d{4,}\/(.+)$`.
...
This module declares ten memory types, nine of which an agent may write, and the SQL restates the same list as a CHECK constraint. It has 13 non-test importers across 4 packages and 2 apps, plus two migrations.

**Producer:** `packages/contracts/src/types.ts:18-82`

**Consumer(s):**

- `packages/index/src/scope.ts:1` imports `MEMORY_TYPES` and `MemoryType`; the scope's `memoryTypes` field is typed by it (`packages/index/src/scope.ts:131`).
...
- `packages/html/src/template.ts:2` imports four of them, `Confidence`, `Importance`, `MemoryType`, and `TaskStatus`, for `NewMemoryInput`; `MemoryStatus` is not imported there.
...
- Every caller of `get` and `all` supplies its own row type parameter `A` and assumes the driver's rows match it, because there is no runtime decode at this boundary. `packages/index/src/retrieval.ts:217-231` declares `HitRow` with snake_case column names and hands it to `all<HitRow>`, so a column rename becomes a silent `undefined` rather than a type error.
...
**Producer:** `packages/html/src/document.ts:29-220`

**Consumer(s):**

- `packages/index/src/project.ts:5` imports `MemoryDoc` and projects it onto rows; `packages/index/src/project.ts:359` reads `doc.links`, and `packages/index/src/project.ts:336` reads `doc.article.codeLangs`.
- `packages/index/src/indexer.ts:8` imports `parseMemory` and `contentHash`, then calls both at `packages/index/src/indexer.ts:305-317`.
- `packages/store/src/store.ts:20` imports `MemoryDoc` as `ReadResult.doc` (`packages/store/src/store.ts:152-156`).
- `apps/mcp/src/handlers.ts:28` imports `MemoryDoc`.
- `packages/html/src/parse.ts:385` declares `parseMemory` returning it.
...
- The indexer assumes a parse failure is a counted skip rather than a fatal error, so one bad file does not stop the tree. `packages/index/src/indexer.ts:305-317` wraps `parseMemory` in `Effect.result`, and `packages/index/src/indexer.ts:35-48` types the skip list on `RebuildReport`.
...
- Consumers assume `href` carries a leading slash and `dst_path` does not. `packages/html/src/document.ts:88-93` states the conversion is `normalizePath` applied at the store boundary, and `packages/index/src/project.ts:360` performs it.
- The indexer assumes `article.html` is a serialization fixed point, because the content hash is computed from it. `packages/html/src/document.ts:130-134` states that re-parsing and re-serializing it yields the same bytes, and `packages/index/src/indexer.ts:311` hashes it.
- Consumers assume `metas.contentHash` is advisory and may disagree with the computed hash. `packages/html/src/document.ts:38-43` states the parser reports it verbatim and never repairs it, so a stale value is visible to `memhtml doctor`.
- Consumers assume `warnings` is non-fatal by construction, so a hand-authored file with an out-of-vocabulary element still parses and still indexes (`packages/html/src/document.ts:213-218`).
...
**Producer:** `packages/store/src/store.ts:225-305`
...
- `tests-integration/tests/contracts.test.ts:34-69` asserts the dedupe behavior through the shipped CLI, which is the store's contract observed from outside.
...
- Batch callers assume the fold owns two properties rather than the caller. The fold produces one commit for N writes, and it assigns paths without collisions. `packages/store/src/store.ts:237-240` states that `freePathFor` reads disk and cannot see a path an earlier op in the same batch has claimed but not written.
- Batch callers assume `skipped` is distinct from a failure, because retrying a skipped op is correct and retrying a failed one is not (`packages/store/src/store.ts:112-114`).
...
- The store assumes dedupe knowledge is injected rather than owned, because the package is SQL-free by design. `packages/store/src/store.ts:188-198` declares `DedupeLookup` as a function and states that the store's only knowledge is that a non-null answer means do not write.
- The store assumes a path move needs an explicit callback, because cross-database foreign keys do not exist. `packages/store/src/store.ts:200-208` declares `MoveCallback` as how `state.access.path` follows an archive.

**Drift risk:** Adding a method that writes without committing, or splitting an existing one-commit operation into two, breaks rollback and stops `diff base..HEAD` from being a reviewable unit. Mitigation: `packages/store/src/store.ts:251-258` and `:264-279` state the one-commit rule for corrections and supersedes. Hold any new operation to the same rule and assert the commit count in the integration tier.
...
- `packages/index/src/git-adapter.ts:3` imports `DiffEntry`, `GitPort`, `StatusEntry`, `TreeEntry` and produces a `GitPort` from a `StoreGitShape` at `packages/index/src/git-adapter.ts:148`.
- `packages/index/src/index.ts:32-43` publishes both the port and the adapter as the package's git surface.
...
- The indexer assumes an `ignored` or `unmerged` status entry is not a change. `packages/index/src/git-adapter.ts:122-130` drops both and states that indexing either side of an unresolved merge would record a state the tree does not agree on.
- The indexer assumes a missing file is a counted skip, not a crash. `packages/index/src/git-adapter.ts:149-164` uses `Effect.catchCause` rather than `Effect.catch` so a defect from a `readFile` wired with `Effect.promise` cannot kill the fiber, and `packages/index/src/git-adapter.ts:74-81` states that an agent listing a path it just archived is the normal case.
- The adapter assumes a submodule entry has no blob behind it and drops it, keeping every sha in the batch resolvable (`packages/index/src/git-adapter.ts:187-193`).
- Both sides assume the tags must differ. `packages/index/src/git-port.ts:77-82` uses `memhtml/IndexGit` because `@memhtml/store` already publishes `memhtml/Git`, and two shapes under one tag would let a layer satisfy the wrong requirement without an error.

**Drift risk:** Adding a `kind` to the store's `StoreChangedPath` or `StoreStatusEntry` union breaks the adapter's exhaustive switches at compile time, which is the intended behavior. Changing the meaning of an existing kind produces no compile error. Mitigation: `packages/index/src/git-adapter.ts:90-95` and `:122-129` export `toDiffEntry` and `toStatusEntry` so the mapping is assertable without a repository, and names `copied` as the case that would corrupt the index and is awkward to provoke from real git.
...
- `apps/cli/src/errors.ts:138-147` assumes a suggestion string names a real command from the table in `apps/cli/src/commands.ts:179`, and enforces that with a walkable record rather than a switch (`:138-142`) so the suite can run every suggestion through the real `parseArgv`. The same comment (`:144-146`) states the import stays out of `apps/cli/src/errors.ts` because it would close the cycle `commands.ts` → `operations.ts` → `errors.ts` (`apps/cli/src/commands.ts:10`, `apps/cli/src/operations.ts:49`; the source comment at `apps/cli/src/errors.ts:145` still carries the older `:8` and `:35`).
...
**Producer:** `packages/index/src/retrieval.ts:51-198` and `packages/index/src/scope.ts:120-218`
...
- `packages/index/src/retrieval.ts:22` imports `assembleScope` and `SearchScope` from the scope module, and `packages/index/src/retrieval.ts:52,172` extends `SearchScope` into both input types.
- `packages/index/src/index.ts:78-92,123-133` publishes `SearchInput`, `SearchHit`, `SearchResult`, `RecallInput`, `RecallPack`, `RetrievalShape`, `QueryEmbedPort`, `SearchScope`, `assembleScope`, `EXCLUDED_BY_DEFAULT`.
...
`SearchScope.facets` is the one field whose wire spelling lives outside the interface. `FacetFilter` (`packages/index/src/scope.ts:36-41`) is a `name` and `value` pair rather than a string, because the SQL binds the halves separately and a predicate builder that re-split its input would carry a second copy of the split rule (`:32-34`). `FACET_SEPARATOR` is `"="` (`:44`); `parseFacetFilter` (`:57-63`) splits one `name=value` spec at the FIRST separator, trims both halves, and returns `undefined` for `=x`, `x=`, and `x`, so a value may contain `=` and a name may not; `parseFacetFilters` (`:66-70`) maps that over a list and drops every rejection silently, the same discipline a blank tag gets, because a scope value that cannot narrow anything must not narrow everything. `packages/index/src/index.ts:127-131` publishes all four, and both doors call the one parser: `apps/cli/src/run.ts:8,271,504` for every `--facet` occurrence on the two commands that take it, and `apps/mcp/src/handlers.ts:29,575,778` for the `facets` parameter of the two tools. A second parser in either app is the drift to watch for: a spec one door accepts and the other drops, with no error on either side.
...
- Consumers assume `entities` is an empty array rather than absent when a memory names none, because a caller reading an absent key cannot tell "no entities" from "this server does not report them" (`packages/index/src/retrieval.ts:84-85`).
- Consumers assume `supersededBy` is present and nullable in every result for the same reason (`packages/index/src/retrieval.ts:88-93`), and that it comes from the `edges` table rather than the head meta, since the meta reaches no SQL column and reading it would cost a file open per hit (`packages/index/src/retrieval.ts:95-97`).
- Consumers assume `score` is comparable only within one result set (`packages/index/src/retrieval.ts:63`), so two searches cannot be merged by score.
- Consumers assume search never fails because the embedder is down, it gets narrower. `packages/index/src/retrieval.ts:255-276` catches the model failure and logs it, and `packages/index/src/retrieval.ts:105-112` surfaces `degraded` so an agent comparing two searches knows one was ranked by fewer signals.
- Consumers assume `scopeEmpty` distinguishes an over-narrow scope from an empty corpus, and is never `true` for an unscoped empty result (`packages/index/src/retrieval.ts:137-138`). `packages/index/src/retrieval.ts:233-249` defines `scopeNarrows` and excludes `includeArchived` because it widens.
- Consumers assume `snippet` is an empty string only when the file has no chunk at all, never absent (`packages/index/src/retrieval.ts:67-74`).
...
**Producer:** `packages/html/src/vocabulary.ts:13-263`
...
- `packages/html/src/constraints.ts:25` imports `REQUIRED_META` and checks each at `packages/html/src/constraints.ts:299-302`, then rejects an out-of-vocabulary meta name at `packages/html/src/constraints.ts:303-306`.
- `packages/html/src/parse.ts` reads the same prefix when collecting head metas and links (`packages/html/src/parse.ts:277-288`).
- `packages/store/src/store.ts:21-31` imports `checkMemory` and `renderTemplate`, so every write passes the vocabulary gate before a file is created.
...
**Drift risk:** Adding an element to `ARTICLE_ELEMENTS` without a matching extraction in `ArticleExtractions` makes the element legal and invisible: it parses without a warning and reaches no index column. Mitigation: pair a vocabulary addition with its extraction field and its projection write, and rely on the graceful-degradation rule at `packages/html/src/vocabulary.ts:100-106`, which makes an unnamed element a warning rather than a refusal in the interim.
...
A third SQL plane sits beside the memory tables and the ATTACHed `state` schema, and it is neither rebuilt nor committed. `TRACE_TABLES` (`packages/index/src/schema-const.ts:61`) names `trace_prompts`, `traces`, and `trace_watermarks`, created inside `index.db` by `packages/index/migrations/0005_traces.sql:8` onward; `schema-const.ts:60` states that a memory rebuild never touches them, and `MEMORY_TABLES` (`:49-58`), the list `packages/index/src/indexer.ts:414` empties, does not name them, so `memhtml index rebuild` keeps every scanned session. `0005_traces.sql:1-6` states the two firewalls: every column is a pointer or a capped head and never a copy of session content, and no retrieval SQL names `traces` or `trace_prompts`. The scanner in `@memhtml/traces` owns the incremental decision as pure arithmetic: `Watermark` (`packages/traces/src/watermark.ts:21-25`, `size`, `mtimeMs`, `byteOff`) mirrors a `trace_watermarks` row, and a `WatermarkAction` (`:38`) of `skip`, `tail`, or `rescan` decides whether a file is opened at all. `packages/index/src/traces-persist.ts:8-22` owns the table and states the scanner's shapes structurally rather than importing them, because `@memhtml/traces` depends on `@memhtml/index` and the arrow stays one way, which is also why the tail merge is an injected function rather than an import.

The plane's root is `Roots.traceRoot` (`apps/cli/src/api-layer.ts:94-101`), resolved once by `layerRoots` (`:107-116`) from `TraceRoot` (`apps/cli/src/config.ts:199-202`, `MEMHTML_TRACE_ROOT` defaulting to `~/.claude`), and read-only. `tests-integration/tests/traces.test.ts:13-31` records what resolving it late costs: `Config` reads against a snapshot of `process.env` taken when the provider is first built, so a suite that assigned `MEMHTML_TRACE_ROOT` in a `beforeAll` after another suite in the same process had resolved config fell back to the real `~/.claude` and scanned 3.67 GB of transcripts into a temp repo. The variable is set at module scope for that reason, and the hook engine applies `--trace-root` through a `ConfigProvider` rather than by mutating `process.env` for the same one.
...
## The composition root and its service shapes

`apps/cli/src/api-layer.ts` is where config becomes services, for both apps. Besides wiring the shapes other packages declare, it declares six `Context.Service` shapes of its own, contract kind 3, and binds each in a layer that reads one environment variable through `effect`'s `Config`. `RootsShape` (`:94-101`) carries `memhtmlRoot` and `traceRoot`, resolved by `layerRoots` (`:107-116`) from `MemhtmlRoot` and `TraceRoot`, with `--repo` winning over `MEMHTML_ROOT`. `EmbedderShape` (`:225-230`) carries the `document` and `query` embed ports, both `undefined` when `MEMHTML_EMBED` is `off` (`:243-246`). `RetrievalPolicyShape` (`:289-294`) carries `vectorCoverageFloor`, read as a number by `VectorCoverageFloor` and refused outside `(0, 1]` with `Effect.die` naming `MEMHTML_VECTOR_COVERAGE_FLOOR` (`:311-316`). `ModelPortShape` (`:357-361`) carries an optional `ModelClientShape`, `undefined` when `MEMHTML_LLM` is `off` (`:367-371`). `ExtractorPortShape` (`:398-402`) carries an optional entity extractor, `undefined` when either `MEMHTML_EXTRACT_ENTITIES` or `MEMHTML_LLM` is `off` (`:407-415`). `ConsolidatorPortShape` (`:442-448`) carries an optional `ConsolidatorShape`, `undefined` when `MEMHTML_LLM` is `off` or `hasConsolidatorCredentials(env)` finds neither a Bedrock credential nor `MEMHTML_LLM_BASE_URL` (`:492-500`). Every optional port reports absence as `undefined` rather than as a failure, so a credential-free environment degrades a search or skips a phase and never turns into a dead CLI (`:222-223`).

`layerConsolidatorPort` (`:486-488`) takes `env: Record<string, string | undefined> = process.env` as a parameter, and `:468-478` states why it has to. `Config` reads through a `ConfigProvider` whose default snapshots `process.env` at module load, while `hasConsolidatorCredentials` reads `process.env` live, so a test that mutated `process.env.MEMHTML_LLM` after importing `effect` would have the two gates disagree about which environment they are in; threading the environment through as an argument makes both injectable from one call, and `apps/cli/tests/consolidator-wiring.test.ts` is where the disagreement produced a false defect before the parameter existed. The same layer requires `RootsShape` for `traceRoot` (`:478-484`) rather than reading `MEMHTML_TRACE_ROOT` a second time, so the tree the consolidator mounts and the tree `memhtml trace index` scans are one resolution of one variable.
...
Every JSON Schema handed to a model is a closed object, contract kind 4. `toInputSchema` (`packages/llm/src/structured.ts:44-50`) and the consolidator's `toJsonSchema` (`apps/consolidator/src/contract.ts:737-766`) both call `Schema.toJsonSchemaDocument(schema, { onExcessProperty: "error" })` (`structured.ts:45`, `contract.ts:738`), and that option is what emits `additionalProperties: false`; without it the document is open, and a model may answer a schema next to the one it was given. `packages/llm/tests/structured.test.ts:31` pins the `false` on a root, and `apps/consolidator/tests/contract.test.ts:1013,1098` pin it on the consolidator's root and on a nested entity. The serialized forms are `StructuredRequest.inputSchema` (`packages/llm/src/model-client.ts:47`), the hand-written override typed as `toInputSchema`'s return, and `CONSOLIDATION_OUTPUT_JSON_SCHEMA` (`apps/consolidator/src/contract.ts:769`), derived once from `ConsolidationPayload` and passed as the turn's `outputSchema`. The decode side is closed the same way: `apps/consolidator/src/client.ts:436,495` and `packages/eval/src/write-path-gate.ts:78` decode with `{ onExcessProperty: "error" }`, because the default strips an undeclared key and succeeds.

The LLM-proxy vocabulary is the one contract in this repo declared in two copies on purpose. `packages/llm/src/proxy-config.ts:37,40,46,52` declares `PROXY_BASE_URL_VAR` (`MEMHTML_LLM_BASE_URL`), `PROXY_API_KEY_VAR` (`MEMHTML_LLM_API_KEY`), `PROXY_MODEL_MAP_VAR` (`MEMHTML_LLM_MODEL_MAP`), and `PROXY_MODEL_PREFIX_VAR` (`MEMHTML_LLM_MODEL_PREFIX`), with `DEFAULT_PROXY_MODEL_PREFIX` `bedrock/` (`:55`) and `PROXY_MODEL_PREFIX_NONE`, the word `none` rather than the empty string because `Config.String` reads `""` as absent (`:57-65`); `ProxyConfig` (`:67-76`) is the parsed result, a `baseUrl`, a nullable `apiKey`, a `modelPrefix`, and a `modelMap`. `LlmConfig` reads all four through `Config` at `packages/llm/src/client.ts:174-180`, and `proxyRouteFor` (`packages/llm/src/proxy.ts:47-52`) picks `embeddings`, `completions`, or `messages` from the `MODELS` row. `apps/consolidator/src/llm-proxy.ts` is a dependency-free copy of the same constants and parser, because the consolidator's agent file is compiled into a published artifact that bundles the workspace packages rather than installing them, so importing `@memhtml/llm` there builds in the workspace and fails `eve build` from an installed tarball (`llm-proxy.ts:8-15`, `proxy-config.ts:15-21`). `apps/cli/tests/llm-proxy-parity.test.ts` pins the two copies to one another; editing one without the other passes type-check and fails that test, which is the intended drift guard.

`OpenAiPromptCache` (`packages/llm/src/wire.ts:69`) is a two-value vocabulary, `off` and `implicit`, read from `MEMHTML_OPENAI_PROMPT_CACHE` (`OPENAI_PROMPT_CACHE_VAR`, `:72`) with `off` as `DEFAULT_OPENAI_PROMPT_CACHE` (`:74`). `openAiPromptCacheFrom` (`:88-98`) trims and lowercases, maps blank to the default, and throws naming the variable and both legal values for anything else, so a typo cannot silently put Bedrock's cache-write premium back on every call while the environment claims to have turned it off. `LlmConfig` applies it at `packages/llm/src/client.ts:170-173`, `ModelClientDefaults.openaiPromptCache` (`packages/llm/src/model-client.ts:78-81`) carries it to the OpenAI lane, and the `CONFIG_VARS` row at `apps/cli/src/config.ts:123-128` is what `memhtml manifest` publishes for it.

The consolidator's answer is a set of `Schema.Class` declarations in `apps/consolidator/src/contract.ts`, and their ceilings are part of the contract. `CandidateEvidence` (`:111`), `CandidateEntity` (`:154-159`, `type` and `name` as non-empty strings, the type deliberately not an enum), `CandidateMemory` (`:170-182`, a `kind` from `CONSOLIDATION_KINDS`, `claim`, `gist`, `entities`, and an `evidence` list with `minLength(2)`), `CandidateCommitment` (`:220`, a `statement` and an `actor` from `COMMITMENT_ACTORS`), `ConsolidationResult` (`:262`), and `ConsolidationPayload` (`:669`, the wire form, with `candidates`, `commitments`, and `readSessionIds` all required). The ceilings are `MAX_QUOTE_CHARS` 600, `MAX_CLAIM_CHARS` 300, and `MAX_GIST_CHARS` 1500 (`:50,53,54`), `MAX_STATEMENT_CHARS` 300 (`:70`), `MAX_CANDIDATES_PER_RESULT` 200, `MAX_COMMITMENTS_PER_RESULT` 200, `MAX_EVIDENCE_PER_CANDIDATE` 32, and `MAX_ENTITIES_PER_CANDIDATE` 64 (`:82-87`), and `MAX_TRANSCRIPTS_PER_RUN` 32 (`:102`); a decode that trips one is an off-contract answer, not a thorough one (`:72-80`). `CandidateMemory` crosses three boundaries: the consolidator decodes the turn's payload at `apps/consolidator/src/client.ts:436,495`, the sleep phase consumes the decoded `candidates` at `packages/sleep/src/phases/trace-consolidation.ts:1018-1030`, and the eval gate re-decodes each raw candidate with the production decoder at `packages/eval/src/write-path-gate.ts:78`, refusing at `door:schema` on failure. Widening a ceiling or adding a field changes what all three accept in one edit, and `CONSOLIDATION_OUTPUT_JSON_SCHEMA` (`:769`) is re-derived from the same class, so the schema the model sees cannot lag the decoder.
...
- **`EmbedPort` and `QueryEmbedPort`**: two narrow ports the index declares over `@memhtml/llm`'s `EmbeddingsShape`, split because the document and query halves use different Bedrock `input_type` values and reusing one silently degrades retrieval (`packages/index/src/indexer.ts:27-32`, `packages/index/src/retrieval.ts:195-198`, `packages/llm/src/embeddings.ts:18-25`).
...
- **`CONFIG_VARS`**: 17 documented environment variables in one array so `memhtml manifest` can describe the whole environment surface and an agent does not have to grep for `process.env`; the last entry's `name` is the imported `MCP_BIN_VAR` rather than a literal, so a rename cannot disclose a variable nothing reads (`apps/cli/src/config.ts:62-181`). Each row is paired with a `Config` read elsewhere: `MEMHTML_ROOT`, `MEMHTML_TRACE_ROOT`, and `MEMHTML_VECTOR_COVERAGE_FLOOR` are `Config.String`, `Config.String`, and `Config.Number` at `apps/cli/src/config.ts:188,199,213`; `MEMHTML_AWS_REGION`, `MEMHTML_OPENAI_PROMPT_CACHE`, and the four proxy variables are `Config.String` inside `LlmConfig` at `packages/llm/src/client.ts:163,170,175,176,179,180`; and `MEMHTML_EMBED`, `MEMHTML_LLM`, and `MEMHTML_EXTRACT_ENTITIES` are `Config.String` at `apps/cli/src/api-layer.ts:243,367,407,411,492`. The row is what `memhtml manifest` publishes and the `Config.*` call is what the process reads; a row with no read discloses a variable nothing honors, and a read with no row is a variable an agent cannot discover.
- **`REINFORCE_SIGNALS`**: the closed three-value vocabulary `positive`, `negative`, `neutral`, declared once in `@memhtml/domain` (`packages/domain/src/reinforce.ts:31-32`) and published to both agent surfaces the way `MEMORY_RELS` and `WRITABLE_MEMORY_TYPES` are: `apps/cli/src/commands.ts:4,540` makes it the `values` of the `--signal` flag with `neutral` as the default, `apps/cli/src/operations.ts:17,177-184` narrows an untrusted string through `decodeSignal`, failing with an `InvalidMemory` whose reason lists the three values, and `apps/mcp/src/tools.ts:16,932` makes it `Schema.Literals(REINFORCE_SIGNALS)` on `memory_reinforce`. `signalValue` (`:39-48`) is a `switch` over the literal union with no default arm, mapping `neutral` to 0 so a neutral reinforcement bumps access without moving the outcome EWMA, and a fourth signal without an arm there is a compile error.
...
Counts are the distinct existing non-docs source paths cited in inline backticks on both pages, measured against the tree at eb9d17a.
