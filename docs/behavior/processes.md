...
Two facts govern how to read every entry. This repository stores no memory: it is the software that manages a separate directory called the memhtml root, located by `$MEMHTML_ROOT` (`apps/cli/src/config.ts:26-31`). The root's git tree is the system of record and `.memhtml/index.db` inside it is a rebuildable projection (`packages/index/src/indexer.ts:17-24`). So a process that writes acts on the root, not on this repo, and the same binary serves many roots.

The primary consumer of every process is a coding agent. There are three initiator families. The CLI publishes 34 commands as one machine-readable table (`apps/cli/src/commands.ts:111-806`), each returning exactly one JSON envelope on stdout with logs on stderr (`apps/cli/src/run.ts:1029-1031`). The MCP server publishes 14 tools (`apps/mcp/src/tools.ts:774-789`) and 2 resource templates (`apps/mcp/src/resources.ts:90`) over stdio. The nightly sleep cycle runs 15 phases as an internal scheduled flow (`packages/sleep/src/contract.ts:17-33`).

Both external doors are thin adapters over one shared use-case module (`apps/cli/src/operations.ts:42-48`, `apps/mcp/src/handlers.ts:33-43`), so a search issued by the CLI and one issued through MCP execute the same query.
...
4. `validate` refuses usage errors: an unknown flag, an unknown command with nearest-match suggestions, a missing required argument or flag, the claim-or-article-html exclusive rule, the exec flag rules, and any value outside a closed vocabulary `apps/cli/src/run.ts:734-795`.
...
2. `toWriteInput` decodes the untrusted memory type, refusing `arc` because an arc is synthesized by sleep, and decodes the two task metas before any file is rendered `apps/cli/src/operations.ts:247-280`.
...
4. `renderChecked` runs the format check over the rendered bytes and refuses with the list of violations before anything is written, staged, or committed `packages/store/src/store.ts:521-528`.
5. The content hash is looked up against the dedupe oracle first, so a duplicate returns the existing path with the tree byte-identical `packages/store/src/store.ts:537-551`.
6. `freePathFor` resolves the placement rule's path, suffixing `-2`, `-3` until one is absent from disk, unless the caller named a valid explicit path `packages/store/src/store.ts:384-406`.
...
8. `reindex` calls `indexer.update()` rather than `indexPaths`, because only the diff-driven path can express a rename and only it records the watermark; then `recordLink` notes the session link and swallows its own failure `apps/cli/src/operations.ts:227-231`, `apps/cli/src/operations.ts:294-296`.
...
1. `decodeApply` judges every JSONL line before any op runs, refusing on the first bad line by number and skipping blank lines while still counting them `apps/cli/src/apply.ts:274-303`.
...
3. When `consolidate: "last-wins"` is set, `planLastWins` folds the ops so a later restatement replaces the content of the earliest slot holding its frame key, and records which stored memories the surviving slots will supersede `apps/cli/src/operations.ts:535-594`.
4. Fold one decodes each planned op through the singular write's own `toWriteInput`; an atomic decode abort returns immediately with every op reported and nothing written `apps/cli/src/operations.ts:683-705`.
5. The optional extraction assist makes one model call over the decoded ops and unions extracted entities into each op's own list; a failure is a logged warning and an unextracted batch `apps/cli/src/operations.ts:718-741`.
6. Fold two is `store.writeMemories`: phase 1 validates every op against the batch's folded dedupe and path-claim state writing nothing, then phase 2 writes every file, stages once, and makes ONE commit, rolling back on any failure `packages/store/src/store.ts:699-820`.
...
8. The store-supersede pass archives every live memory a surviving slot displaced in one `supersedeMemories` call, then reindexes again because archive paths moved; a failure here is annotate-only `apps/cli/src/operations.ts:782-811`.
...
3. `sanitizeFtsQuery` reduces the caller's prose to indexable terms, because an apostrophe or a leading hyphen is a hard driver error rather than an empty result `packages/index/src/retrieval.ts:231`.
4. `activeArms` drops any arm whose precondition is absent, which is the whole degradation mechanism: the vector arm needs the query vector, the salience arm needs the attached state plane, the lexical arm needs surviving query terms `packages/index/src/retrieval-sql.ts:263-270`.
...
1. `guardEmbedModel` refuses when the stored vector space disagrees with the configured one, before any write, so an index can never accumulate rows under two models `packages/index/src/indexer.ts:212-218`.
2. The recorded watermark is read; when it is absent there is no index at all, so `update` falls through to a full `rebuild` rather than diffing against nothing `packages/index/src/indexer.ts:534-552`.
...
4. Every committed diff target's blob is read in two subprocesses rather than two per file, because a per-file tree walk was the term that made bulk ingest quadratic `packages/index/src/indexer.ts:572-589`.
...
7. Writes are applied in bounded batches and the watermark is recorded, which is what makes "the index describes the current commit" answerable `packages/index/src/indexer.ts:668-669`.
...
3. The branch is created BEFORE any phase runs and every commit lands on it, so `main` is never touched by a run; a dry run creates no branch `packages/sleep/src/run.ts:95-97`.
4. `recordRun` writes the run row as `running`, through a wrapper that never lets a reporting failure fail the run `packages/sleep/src/run.ts:98-108`.
...
7. `preflight` runs first: it refuses a dirty tree, refreshes the index so every later phase reads current rows, and commits nothing `packages/sleep/src/phases/preflight.ts:20-41`.
...
1. The `sleep merge` dispatch arm composes the discrimination gate here, in the CLI, because `@memhtml/sleep` cannot import the eval and must not be able to default it silently `apps/cli/src/run.ts:494-524`.
2. `--skip-gate` logs a warning and passes no gate, which is a deliberate visible override rather than a default `apps/cli/src/run.ts:498-502`.
3. `resolveRun` reads the named run row, or the newest recorded one; a missing row refuses with `no-run` `packages/sleep/src/review.ts:29-35`, `packages/sleep/src/review.ts:231-240`.
4. The target branch is checked out and its head read, before anything moves `packages/sleep/src/review.ts:242-246`.
5. Refusal one: `main` having advanced past the run's `base_sha` means the run curated a corpus that no longer exists, so the merge refuses with `main-advanced` `packages/sleep/src/review.ts:248-259`.
6. Refusal two: the pre-merge gate is run under `Effect.result` and a failure becomes `gate-failed` with `main` never moving `packages/sleep/src/review.ts:261-273`.
7. `discriminationGate` runs the probes in `fake` mode and fails when the report does not pass, which is what makes a retrieval regression block the merge `packages/eval/src/run.ts:174-181`.
8. A fast-forward only, never a merge commit; on success the run row is rewritten as `merged` `packages/sleep/src/review.ts:275-301`.
...
2. `layerServer` merges the toolkit and the two resources over the CLI's own app layer, so both doors resolve to one database file, one git root, and one vector space `apps/mcp/src/server.ts:39-54`.
3. `Logger.LogToStderr` is set here because stdout on this transport is the NDJSON-RPC stream and one log line would corrupt the frame a client is mid-parse on `apps/mcp/src/server.ts:53`.
...
6. Each handler renames snake_case wire parameters to the operations layer's camelCase, mapping an explicit `null` to absent, then calls the same shared use case the CLI command calls `apps/mcp/src/handlers.ts:102`, `apps/mcp/src/handlers.ts:524-556`.
7. `authored` resolves the body-or-article-html exclusive rule at the wire boundary, refusing both-supplied and neither-supplied rather than picking one `apps/mcp/src/handlers.ts:143-167`.
8. `handled` maps every typed error through `toToolFailure` into the `ToolFailure` shape each tool declares, which is the branch that passes the message through verbatim instead of the framework's generic internal-error sentence `apps/mcp/src/handlers.ts:70-71`, `apps/mcp/src/failure.ts:1`.
...
- Archive a memory: entry at `apps/cli/src/operations.ts:1035`. A soft eviction: `git mv` into `archive/<YYYY>/` with the archive stamps applied at the destination, then one diff-driven reindex `packages/store/src/store.ts:888`.
- Link two memories: entry at `apps/cli/src/operations.ts:1022`. Decodes the rel against the authorable set, refuses an edge whose class disagrees with its endpoints' types, and is idempotent on the pair so a re-link commits nothing `packages/store/src/store.ts:1037`.
...
- List memories or tasks: entry at `apps/cli/src/operations.ts:1193` and `apps/cli/src/operations.ts:1444`. Direct indexed scans with keyset pagination on `path`, deliberately not ranked retrieval; the task scan carries each row's `blockedBy` from one correlated subquery over `edges`.
- Walk a memory's neighbourhood: entry at `apps/cli/src/operations.ts:1093`. Two fixed-depth joins in a `UNION ALL` rather than a recursive CTE, both edge directions, bounded at two hops and filtered to `edge_class = 'memory'`.
...
- Search transcripts: entry at `apps/cli/src/operations.ts:1606`. FTS over session first-prompts and AI titles through the same sanitizer the memory arms use; names no memory table, which is the trace firewall in that direction.
- Read memory-session links: entry at `apps/cli/src/operations.ts:1674`. Answers from either side and refuses a call naming neither, rather than scanning every link ever recorded.
...
- Regenerate AGENTS.md: entry at `apps/cli/src/agents-doc.ts:215`. Renders the doc from the command table and either writes it or, under `--check`, refuses on drift without writing.
...
- Run the discrimination eval: entry at `packages/eval/src/run.ts:77`. Generates a fixture corpus, runs the probes, and reports three distinct outcomes so a live run skipped for missing credentials can never look like a pass.
...
- Read an MCP resource: entry at `apps/mcp/src/resources.ts:42` and `apps/mcp/src/resources.ts:67`. `memhtml://file/{path}` returns one memory's title, claim, and body for citation-grade drill-down and bumps salience through the same `readMemory` the tool calls; `memhtml://sleep/{run-id}` reads the run's committed HTML report from the tree.
