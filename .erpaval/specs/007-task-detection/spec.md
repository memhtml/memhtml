...
- AC-3-2 (dedup-merge) Proposed pairs at/above NEAR_DUPLICATE_THRESHOLD that mergeVetoed excludes (re-applied per pair to separate true vetoes from the residual) mint `review:` tasks quoting both divergent texts via `<q cite="<path>">` (was blockquote; corrected at implementation — see the design decision at the top of this spec). universeComplete = model arm ran AND zero isolated failures AND no truncation at DEDUP_PAIR_LIMIT, DEDUP_MAX_COMPONENTS, or packGroups.
