...
index database, and the finding was recorded as **"the index's lock is exclusive."**
...
(`apps/cli/tests/apply.test.ts:746-752`). The drift is mechanically impossible rather than
discouraged.
...
- not "`PRAGMA foreign_keys = OFF` suppresses the cascade" but "it suppresses the cascade **outside** a
  transaction and is a **no-op inside** one, which is where a migration runs"
