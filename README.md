...
memhtml serve mcp                             # the same store over stdio: 18 tools, 3 resources
```
...
2. The MCP server. `memhtml serve mcp` speaks stdio and exposes 18 tools and 3 resources over the same repo: write, read, search, recall, correct, link, archive, batch writes, and trace search. A CLI command and a running server share one store, because WAL admits one writer and any number of readers, and a contended write retries on `SQLITE_BUSY` (see `RUNBOOK.md`, section 4).
...
| `@memhtml/mcp`          | The `memhtml-mcp` stdio server: 18 tools, 3 resources.                                                              |
