 ...
 *
 * Pure and seeded, so the same seed yields byte-identical files on any machine: the whole point of a
 * fixture the discrimination gate runs against is that a change in the numbers means a change in the
 * RANKING, never a change in the corpus. Nothing here reads a clock, a filesystem, or an environment
 * variable — `fixture.ts` is the only module that writes.
 *
...
export interface MemorySpec {
  ...
  /** The one load-bearing sentence: becomes the `<mark>` span and therefore `files.gist`. */
...
/**
 * One memory's durable access bookkeeping — a `state.access` row.
 *
 * Part of the SPEC rather than something the harness invents, because the salience arm reads this plane
 * and a fixture whose state plane is empty cannot measure that arm at all. Seeded and query-blind: the
 * spread is a function of the corpus seed and nothing here consults the probe list.
 */
 ...
 *
 * Disjoint matters: the deterministic embedder is a bag of words, so two topics sharing nouns would
 * put unrelated memories inside a probe's candidate window and the gate would be measuring vocabulary
 * overlap rather than discrimination.
 */
...
/** Words a probe query drops. A query is what an agent types, not the claim pasted back. */
...
/**
 * A probe query: the target's own content words, in order, capped.
 *
 * Content words rather than the claim verbatim, and this is the one place the query design is
 * load-bearing. A NUMERIC-family control differs from its target in exactly one numeric token, so a
 * query that dropped the number would have identical overlap with both and the vector arm could not
 * order them at all — the probe would measure a tie-break. Keeping the digits is therefore not a
 * concession to make the gate easy: it is what makes the numeric family a probe rather than a coin
 * toss. The same reasoning keeps a variant qualifier's ANCHOR in the query while the qualifier
 * itself, which only the control carries, stays out.
 *
 * Stopwords go because `not` must not be able to hide behind them: a query carrying the target's
 * function words would raise the negation control's lexical overlap for a reason unrelated to the
 * fact either states.
 */
 ...
 *
 * Each kit returns markup that follows the claim paragraph. None of them contains a `<mark>` — the
 * claim leads the article and constraint 5 forbids one inside an `<aside>` or `<details>`.
 */
const KITS: ReadonlyArray<(topic: Topic, ordinal: number) => ReadonlyArray<string>> = [
  ...
  // `<dfn>` — promoted to a `concept:` entity by the indexer.
  (topic) => [
 ...
 * markup, and `renderTemplate` escapes its `body` strings as text — which is correct for a tool
 ...
 *
 * **Distinct per ordinal, and that is a requirement rather than a nicety.** The topic list, the type
 * list, the noun list, and the verb list all cycle, so `(topic, type, noun, verb)` repeats every
 * lcm(12, 9, 5, 3) = 180 ordinals — and a corpus of 200 therefore held 20 pairs of memories asserting
 * the SAME claim in different directories. A probe's query is derived from its target's claim, so a
 * shared claim means the query identifies two memories equally well: the twin outranks the target on
 * recency about half the time, and the probe reports an inversion that says nothing about the control
 * it was built to test. Measured on the first generated 200: the twin took fused rank 1 and the target
 * rank 8 on the probe that surfaced it.
 *
 * `scopeFor` is what disambiguates — a per-ordinal environment noun spliced into the claim, so two
 ...
 * controls were still correctly ranked below the target, so this is not an inversion: it is a fixture
 * that cannot measure rank 1, which caps MRR near 0.25 no matter how well retrieval works.
 *
...
/**
 * A per-ordinal environment noun: `staging`, `canary`, `frankfurt`, …
 *
 * Digit-free, so a `numeric` control still differs from its target by exactly the quantity the claim
 * asserts and by nothing else. A second axis of distinction alongside {@link nounIndexOf}: 23 is
 * coprime with both 12 and 9, so the scope word advances on every ordinal without ever re-synchronizing
...
/**
 * A base-26 letter code for an ordinal: `a`, `b`, … `z`, `ba`, `bb`, …
 *
 ...
 * of `(topic, type, ordinal mod k)` for several small `k`, which means two ordinals a common multiple
 * apart produce IDENTICAL article text under different titles. `files_content_hash_active` is a
 ...
 *
 * A code rather than widening the claim vocabulary, because the claim is what a probe query is derived
 * from — pushing the distinguisher into the claim would put a token unique to one memory into the query
 * that is supposed to discriminate it from its controls, which the controls copy, and the probe would
 * then be measuring a shared unique token.
 */
 ...
 *
 * `procedural` and `semantic` only, and the reason is finding #33: `negationDivergent` is a
 * marker-PRESENCE check over the whole text, so a target whose body already says `no`, `not`,
 * `fails`, or `invalid` puts a marker on BOTH sides of the pair and the polarity flip becomes
 * invisible to the predicate that defines the family. The claims and bodies these two types generate
 * are affirmative by construction — and {@link buildProbes} still re-checks each pair through
 * `deriveControl`, so a future edit that smuggles a marker into a body drops the control rather than
 * shipping a paraphrase dressed as an adversary.
 */
 ...
 *
 * **The family marker LEADS the title, and that placement is load-bearing.** The title is the slug and
 * the slug is the filename, so a trailing marker made every control's path an extension of its target's
 * stem — `…-110-qualified-variant.html` against `…-110.html` — and `-` (0x2D) sorts before `.` (0x2E),
 * so the control's path sorted before its target's on EVERY pair.
 *
 ...
 * tie-break then went to the CONTROL every time — a systematic loss decided by filename punctuation.
 *
 * Leading the marker makes the ordering depend on the marker word against the target's first word,
 * which is arbitrary per pair rather than adverse — the relationship a real corpus has, where a memory
 * and its near-twin carry unrelated titles. It is deliberately not tuned the other way: a fixture that
 * guaranteed the target won every tie would be a fixture arranged to pass.
 */
 ...
 * the person files.
 *
 * The seeded PRNG decides only the JITTER — confidence, importance, which memories carry a TTL, and
 * the day offsets. Placement and vocabulary are functions of the ordinal, so a corpus is legible: the
 * fifth `checkout-api` memory is always the fifth `checkout-api` memory.
 */
 ...
 *
 * The same noun the claim uses, through {@link nounIndexOf}, so the title and the claim name one
 * subject: `title` is the first field of `fts_text`, and a title naming a different noun than the
 * claim would put a term into the lexical arm that the memory does not actually assert.
 */
...
/** A synthetic session uuid. Shaped like a real one; derived from the ordinal, so reproducible. */
 ...
 * "doctor clean on the fixture" criterion unmeetable — and an edge invented against a path that was
 * never written proves nothing about the edge encoding.
 */
 ...
 * an EARLIER version of the fact rather than the same one — which is also what makes the supersedes
 * edge mean something.
 */
 ...
 *
 * **Targets are drawn EVENLY across the candidate list, not off the front, and that is a correctness
 * requirement rather than an aesthetic one.** Two of the four ranking arms — recency (w 0.5) and
 * salience (w 0.4), together 31% of the fold's weight — are QUERY-BLIND: they rank a fixed
 * `DEFAULT_ARM_LIMIT` window of the corpus whatever was asked. `base` is generated in ordinal order and
 * a memory's `updatedAt` advances with its ordinal, so taking the first N candidates takes the N OLDEST
 * memories — every probe target then sits outside the recency window by construction, and loses both
 * blind arms on every probe. Probed directly: the recency window held ordinals 157-199 while the probe
 * targets were 1-155, an overlap of exactly zero, and MRR capped at 0.06 with the inversion check
 ...
 * hit. It changes NOTHING about the controls or the strict per-probe check — the inversion count was
 * already 1 before and after — it makes the MRR aggregate a measurement of ranking rather than of where
 * the generator happened to slice.
 */
const buildProbes = (
   ...
   *
   * The stride pass is what spreads the ages; the fall-through keeps the function total when a family
   * refuses a control on a strided pick, so a corpus still yields `wanted` probes rather than however
   * many the stride happened to land on.
   */
  ...
  for (const target of ordered) {
    ...
    for (const family of DIVERGENCE_FAMILIES) {
      ...
      controls.push({
        ...
        links: [],
        /**
         * **The target's element kit is COPIED, not dropped, and this is what makes the pair a fair
         * test.** A control without the kit is a strictly SHORTER document carrying the same query
         * terms — and both ranking arms are length-sensitive: the FTS arm's only relevance signal is
         * MATCH's own term-density order, and a vector is L2-normalized so unrelated tokens dilute
         ...
         * document length, not on the fact — a control that wins by being terser proves nothing about
         * discrimination, and a fold that "fixed" it would be tuned against an artefact.
         *
 ...
 *
 * Order is fixed — people, base, archive tier, controls — so the generated tree is written in a
 * stable order and two runs at one seed produce byte-identical files.
 */
 ...
 *    evidence that a wrong fact had been useful. This is what a synthetic control IS, and it is also
 *    why the rule cannot be read as tuning toward the probes: the exclusion is by ROLE, decided when
 *    the control is minted, not by whether it happens to be some probe's control.
 ...
 *
 * The distribution is a long tail — `1 / (floor + jitter)` — because that is the shape retrieval
 * traffic has: a handful of memories are read constantly and most are read once. A uniform count would
 * make `ln(1 + accessCount)` nearly constant and the term would carry no ordering information.
 */
const buildAccess = (
  ...
  // jitter stream `buildBase` consumed — a shared generator would make a corpus-size change silently
  // re-roll every access count.
