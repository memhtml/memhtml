 ...
 * consolidator, off a read-only mount inside its sandbox — and what comes back is a distilled CLAIM,
 * never a span of transcript. What this phase SENDS is a manifest of metadata: paths, spans, counts,
 ...
 *
 * **A watermark means a transcript was READ, never that one was requested.** The phase asks about a
 * batch and watermarks only what the agent reports having reached, intersected with that batch — see
 * `markSessionsConsolidated`'s call site. The distinction is load-bearing because
 * `trace_consolidations` is an anti-join: a watermark on a session whose transcript never arrived
 ...
 * assembled statement to prove it. A trace row is incapable of entering RRF; a memory this phase
 * synthesized is indistinguishable from one an agent wrote, which is correct — it IS one.
 *
 ...
 *
 * **Degrades three ways, fails none of them.** No consolidator bound, a consolidator that failed
 ...
 * credentials is not a broken night, and a run that lost this phase must not go red.
 */

/**
 * The smallest transcript worth a model's attention, in bytes.
 *
 * 8 KiB, measured rather than picked. Over the live corpus at `~/.claude/projects` on 2026-08-08
 ...
 * distilled from a 10-line file could only ever restate one of those lines, which
 * `apps/consolidator/agent/instructions.md:42-43` names as below the bar anyway — so this saves the
 * call rather than changing the answer.
 */
 ...
 *
 * Ten, which is below the consolidator's own 32-transcript ceiling
 * (`apps/consolidator/src/contract.ts:210`) and deliberately so: that cap bounds RESIDENT BYTES in
 ...
 *
 * The two caps compose rather than duplicate: whichever is smaller binds, and the consolidator warns
 ...
 * half-finished and then watermarked as done — the interesting part arriving after the row that says
 * it was handled. The cutoff is derived from `env.at`, which is midnight of the run's own date
 * (`run.ts:56-60`), NOT from a clock: a phase that read wall-clock could not be tested against a
 * fixed date, and `env.ts:60-67` states the rule. The consequence is worth naming — with `at` at
 * midnight, nothing written on the run's own date is eligible, so the quiet window in practice
 * subsumes the live-session guard rather than merely satisfying it.
 */
 ...
 *   that reached it unchecked would fail the whole commit rather than skip one candidate. Checked
 ...
 *   deliberate — a scripted or future consolidator that did not would still not get past here.
 * - **A claim that slugs to nothing.** `slugify` folds to `[a-z0-9-]`, so a claim written entirely
 *   in CJK, Cyrillic, or punctuation reduces to `SLUG_FALLBACK` and every such candidate — on
 *   unrelated subjects, from unrelated sessions — files under one stem. Under the disk-authoritative
 ...
 *   reviewer or a later correction can address. The consolidator writes English prose, so this is a
 *   gate on a value it should never send rather than a filter on ordinary output.
 */
 ...
 *
 * Derived rather than asked for, and that is the same decision `arc-synthesis` makes about a slug: a
 ...
 * night's memories over a failed note about them would invert the priority — the same
...
export const traceConsolidation: PhaseBody = (env) =>
  Effect.gen(function* () {
    /**
     * The v1 counters survive, and they are not vestigial. The count of sessions with no memory
     * linked to them is the one number that says whether the agent is writing memories at all, and
     * that question is orthogonal to whether this phase has read a transcript — a session can be
    ...
    /**
     * The cutoff carries its MILLISECONDS, and dropping them is a hole rather than a rounding.
     *
     ...
     * and the model call is what does not happen. Stopping before the call rather than after it is
     * the point — a dry run that spent Opus tokens to then discard the answer would be the most
    ...
    /**
     * The whole consolidator call in isolation. A failure is a VALUE here rather than a phase
     ...
     *
     * `Effect.result` rather than a `catch` that returns a neutral value, because the two outcomes
     * need different report lines: "the agent found nothing" and "the agent could not be asked" are
     * both `consolidated: 0` and an operator must be able to distinguish them.
     */
    /**
     ...
     * not in a transcript's bytes: which project a session ran under, how long it lasted, and — the
     * one worth a join — which memories the corpus already links to it, since a pattern already
     * written down is not the new signal the bar asks for. `sessionManifestRows` is the query.
     *
     * **A manifest lookup failure degrades to the bare batch rather than failing the phase.** The
     * manifest sharpens the ask; the transcripts are the ask. Same posture `frameConflicts` takes for
     * the same reason — losing the night's memories over a failed note about them inverts the
     * priority — and the fallback is still a complete `{sessionId, filePath}` per session, so a
     * degraded run reads the same transcripts with less context rather than reading fewer.
     */
    ...
    for (const [offset, candidate] of candidates.entries()) {
       ...
       * and the alternative — one fixed overflow path — is the overwrite this probe exists to
      ...
      /**
       * A frame conflict does NOT suppress the write, and that is INV-1 rather than an oversight: the
       * assist proposes and never blocks, because sometimes the contradiction IS the answer — a
      ...
      yield* writeFileBytes(
        ...
        renderTemplate({
          ...
          /**
           * The candidate's supporting detail becomes the body. NOT its evidence quotes — those are
           * transcript spans, and copying one into an article would put session content in the
     ...
     *
     * ## Never a session whose transcript never arrived
     *
     ...
     *
     * **The guard is structural rather than a check placed here.** `ConsolidationOutcome` cannot be
     * constructed without `analyzedSessionIds` (`../consolidator.ts`), so there is no shape a
     * consolidator can return that leaves this phase with only the batch to fall back on — which is
     * what a `?? batch` default, or an optional field, would have reintroduced.
     *
     ...
     * ## Last, not first
     *
     * A process killed between the commits and this write reconsolidates those sessions next night — a
     * wasted model call and a duplicate candidate a reviewer declines — while the reverse order would
     * lose the transcripts silently, marked read with no memory to show for it.
     */
     ...
     * report is the operator-visible signal that transcripts went missing — the state that previously
     * had no reading at all, since a watermark over the batch made them equal by construction.
     */
 ...
 * consolidator is an injected collaborator — the real one an eve agent over HTTP, a scripted one in
 ...
 * nobody asked about. Intersecting makes the outcome able to NARROW the batch and never to widen it,
 * which is the only authority it needs.
 *
 * Batch order is preserved rather than the outcome's, so the watermark writes newest-first exactly as
 * the selection read — the ordering that makes a report line and a test's `toEqual` reproducible.
 */
const analyzedFrom = (
 ...
 * link — a session with three linked memories is three rows, one with none is a single row carrying
 ...
 *
 * A session in the batch with NO manifest row also falls back to its bare ref rather than being
 * dropped. That gap is possible — `unconsolidatedSessions` and this lookup are two statements — and a
 * session silently missing from the handover would be one the phase decided not to read while counting
  ...
  Effect.gen(function* () {
    ...
    return batch.map((session) => {
      ...
      return {
        ...
        /**
         * The path comes from the SELECTION's row rather than the manifest join's, so the file handed
         ...
         * corpus holds nothing for a session whose findings were never written down — and distinct
 ...
 * identical stem, so a repeat claim silently overwrote whatever occupied it: a memory a human had
 ...
 *
 * `store.freePathFor` itself is NOT reused, and the reason is reach rather than preference: it is a
 ...
 * the probe read through `readFileBytes` — the sleep package's own repo-relative reader, the one
